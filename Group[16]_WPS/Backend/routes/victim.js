...
// ---------------------------------------------------------------
// SHARED HELPER — Dispatcher Selection
// Returns { dispatcherId, noteAddendum }
// If the victim already has a previous request and that dispatcher
// is still available, re-assign them (noteAddendum = null).
// If the previous dispatcher is no longer available, falls back to
// round-robin and sets noteAddendum to record the handoff context.
// excludeId is used when the victim is themselves a dispatcher, to
// prevent self-assignment.
// ---------------------------------------------------------------
async function selectDispatcher(conn, victimId, excludeId = null) {
    let noteAddendum = null;

    // Check if victim already has a dispatcher from a previous request
    const [activeRequest] = await conn.query(
        `SELECT Dispatcher_id FROM Request_log
         WHERE Victim_id = ?
    ...
    if (activeRequest.length) {
        const prevDispatcherId = activeRequest[0].Dispatcher_id;

        // Only re-use if they are still available and not the excluded id
        if (prevDispatcherId !== excludeId) {
            const [dispAvail] = await conn.query(
                `SELECT User_id FROM Dispatcher WHERE User_id = ? AND Availability = 'Yes'`,
                [prevDispatcherId]
            );
            if (dispAvail.length) {
                logger.info(`Re-assigning previous dispatcher ${prevDispatcherId} to victim ${victimId}`);
                return { dispatcherId: prevDispatcherId, noteAddendum: null };
            } else {
                // Previous dispatcher exists but is no longer available — record handoff context
                noteAddendum = `[Previously handled by Dispatcher ${prevDispatcherId}, reassigned due to unavailability]`;
                logger.info(`Previous dispatcher ${prevDispatcherId} unavailable for victim ${victimId}, falling back to round-robin`);
            }
        }
    }
    ...
    if (!dispatchers.length) return { dispatcherId: null, noteAddendum: null };
    ...
    return { dispatcherId: dispatchers[nextIndex].User_id, noteAddendum };
}
...
router.post('/emergency', async (req, res) => {
    ...
    try {
        ...
        const { dispatcherId, noteAddendum } = await selectDispatcher(conn, user_id, null);
        if (!dispatcherId) {
            await conn.query('ROLLBACK');
            return res.status(503).json({ message: 'No dispatchers available. Please call emergency services directly.' });
        }

        const finalNote = noteAddendum ? `${note} ${noteAddendum}` : note;
        ...
        await conn.query(
            ...
            [requestId, user_id, dispatcherId, finalNote, location]
        );
...
router.post('/emergency/dispatcher', async (req, res) => {
    ...
    try {
        ...
        // Pass user_id as excludeId so a dispatcher cannot be assigned their own request.
        // selectDispatcher will also check for a previous dispatcher first (excluding self).
        const { dispatcherId, noteAddendum } = await selectDispatcher(conn, user_id, user_id);
        if (!dispatcherId) {
            await conn.query('ROLLBACK');
...
router.post('/request', async (req, res) => {
    ...
    try {
        ...
        const { dispatcherId, noteAddendum } = await selectDispatcher(conn, req.user.id, null);
        if (!dispatcherId) {
            await conn.query("ROLLBACK");
        ...
        await conn.query(
            `INSERT INTO Request_log (Request_id, Victim_id, Dispatcher_id, Request_time, Note, Location, Type) 
             VALUES (?, ?, ?, CURRENT_TIMESTAMP, ?, ?, 'Emergency')`,
            [Request_id, req.user.id, dispatcherId, finalNote, Location]
        );
        logger.info(`Inserted Request_log: ${Request_id} | Victim: ${req.user.id} | Dispatcher: ${dispatcherId}`);

        await conn.query("COMMIT");
        logger.txCommit('POST /victim/request', `Request ${Request_id} submitted`);

        res.json({ message: 'Request submitted successfully', Request_id, Dispatcher_id: dispatcherId });
