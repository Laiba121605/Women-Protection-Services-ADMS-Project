...
// ======================================================
// 🔹 GLOBAL WRITE LOCK CHECK
// ======================================================

const requestIsAdmin = (req) =>
    req.user && req.user.roles && req.user.roles.includes('Admin');

function globalWriteLockStatus(req) {
    const { adminActive, adminLockExpiry, adminLockHolder } = getAdminLockState();
    if (!adminActive || adminLockExpiry <= Date.now()) return { locked: false };
    if (req.user.id === adminLockHolder) return { locked: false };
    return { locked: true, lockedBy: adminLockHolder };
}

function enforceGlobalWriteLock(req, res, next) {
    const writeMethods = ['POST', 'PUT', 'DELETE', 'PATCH'];
    if (!writeMethods.includes(req.method)) return next();

    const lock = globalWriteLockStatus(req);
    if (lock.locked) {
        logger.warn(`WRITE_BLOCKED_BY_GLOBAL_LOCK - User: ${req.user.id}, Method: ${req.method}, Path: ${req.path}, LockedBy: ${lock.lockedBy}`);
        return res.status(423).json({
            error: `The system is currently locked for maintenance by an administrator (${lock.lockedBy}). All write operations are temporarily disabled. Please try again later.`,
            locked_by: lock.lockedBy,
            retry_after: 'Check back shortly or contact the administrator.',
        });
    }
    next();
}

router.use(enforceGlobalWriteLock);
...
// CHANGED: removed Status filters — Police_services and Ambulance_service have no Status column
...
// CHANGED: removed Status updates for Police_services and Ambulance_service
...
const assignVolunteers = async (connection, assignmentId, volunteerIds, dispatcherId) => {
    ...
    for (const volunteerId of volunteerIds) {
        ...
        try {
            ...
            if (!vol.length) throw new Error(`Volunteer ${volunteerId} not found.`);
...
router.get('/victim/search', async (req, res) => {
    const { phone, user_id, email } = req.query;

    if (!phone && !user_id && !email)
        return res.status(400).json({ error: 'Provide at least one search param: phone, user_id, or email.' });

    try {
        const conditions = [];
        const params     = [];
        if (phone)   { conditions.push('u.Phone_no = ?'); params.push(phone); }
        if (user_id) { conditions.push('u.User_id = ?');  params.push(user_id); }
...
router.post('/victim', async (req, res) => {
    const { name, phone_no, email, address, date_of_birth, cnic, password, emergency_contact } = req.body;
    const connection = await pool.getConnection();
    try {
        ...
        let userId, isNewUser = false, isEmergency = false;

        if (existingPhone.length) {
            userId = existingPhone[0].User_id;
            const [existingVictim] = await connection.query('SELECT User_id FROM Victim WHERE User_id = ?', [userId]);
            if (!existingVictim.length) {
                const ec = emergency_contact || phone_no;
                await connection.query(`INSERT INTO Victim (User_id, Emergency_contact) VALUES (?, ?)`, [userId, ec]);
            ...
            const rawDigits = parseInt(userId.substring(0, 13), 16).toString().padStart(13, '0').substring(0, 13);
            ...
            const finalPass = password ? await hashPassword(password) : crypto.randomBytes(8).toString('hex');
            await connection.query(
                `INSERT INTO User (User_id, Name, Email, Phone_no, Address, Password, Date_of_Birth, CNIC) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
                [userId, name, finalEmail, phone_no, address || null, finalPass, finalDOB, finalCNIC]
            );
            const ec = emergency_contact || phone_no;
            await connection.query(`INSERT INTO Victim (User_id, Emergency_contact) VALUES (?, ?)`, [userId, ec]);
        ...
        res.json({ message: isNewUser ? 'New victim created.' : 'Existing victim found.', victim_id: userId, is_new: isNewUser });
    } catch (err) {
...
router.put('/victim/:id', lockRow('Victim', 'id'), async (req, res) => {
    ...
    try {
        const [userRows] = await req.dbConnection.query('SELECT * FROM User WHERE User_id = ?', [victimId]);
        ...
        if (email) {
            const [dup] = await req.dbConnection.query('SELECT User_id FROM User WHERE Email = ? AND User_id != ?', [email, victimId]);
            if (dup.length) throw new Error('Email already registered to another user.');
            userUpdates.push('Email = ?'); userParams.push(email);
        }
        if (phone_no) {
            if (!/^\d{10,11}$/.test(phone_no)) throw new Error('Phone number must be 10-11 digits.');
            const [dup] = await req.dbConnection.query('SELECT User_id FROM User WHERE Phone_no = ? AND User_id != ?', [phone_no, victimId]);
        ...
        if (userUpdates.length) {
            ...
            await req.dbConnection.query(`UPDATE User SET ${userUpdates.join(', ')} WHERE User_id = ?`, userParams);
        }
        if (emergency_contact) {
            if (!/^\d{10,11}$/.test(emergency_contact)) throw new Error('Emergency contact must be 10-11 digits.');
            if (emergency_contact === (phone_no || user.Phone_no)) throw new Error('Emergency contact cannot be the same as personal phone.');
            await req.dbConnection.query('UPDATE Victim SET Emergency_contact = ? WHERE User_id = ?', [emergency_contact, victimId]);
        }
...
router.get('/calls/all', async (req, res) => {
    try {
        const [calls] = await pool.query(`
            SELECT
                ...
                i.Severity,
                i.Emergency_type,
                d_user.Name        AS dispatcher_name,
        ...
        `);
        logger.info(`GET_ALL_CALLS - User: ${req.user.id}, Count: ${calls.length}`);
        res.json(calls);
    } catch (err) {
        logger.error(`GET_ALL_CALLS_FAILED: ${err.message}`);
        res.status(500).json({ error: err.message });
    }
});
...
router.post('/call-log', async (req, res) => {
    ...
    try {
        ...
        res.json({
            ...
            workload_warning: load.hasActiveIncidents ? `You currently have ${load.activeCount} active incident(s).` : null,
...
router.post('/incident', async (req, res) => {
    const { request_id, emergency_type, severity, note, verification_status = 'True', location } = req.body;
    const connection = await pool.getConnection();
    let locked = false;
    ...
    try {
        ...
        if (!VALID_EMERGENCY_TYPES.includes(emergency_type)) throw new Error(`Invalid emergency type. Options: ${VALID_EMERGENCY_TYPES.join(', ')}`);
        if (!VALID_SEVERITY.includes(severity)) throw new Error(`Invalid severity. Use: ${VALID_SEVERITY.join(', ')}`);
        if (!VALID_VERIFICATION_STATUS.includes(verification_status)) throw new Error(`Invalid verification_status. Use: ${VALID_VERIFICATION_STATUS.join(', ')}`);
        ...
        const [existing] = await connection.query('SELECT Incident_id FROM Incident WHERE Request_id = ?', [request_id]);
        ...
        await connection.query(`
            INSERT INTO Incident (Incident_id, Request_id, Emergency_type, Severity, Note, Location, Verification_status, Status)
            VALUES (?, ?, ?, ?, ?, ?, ?, 'Ongoing')
        `, [incidentId, request_id, emergency_type, severity, note || request.Note, incidentLocation, verification_status]);

        await connection.query('COMMIT');
        logger.info(`INCIDENT_CREATED - Incident: ${incidentId}, Request: ${request_id}, Dispatcher: ${req.user.id}`);

        res.json({
            message: 'Incident created successfully.', incident_id: incidentId,
            victim: { id: request.Victim_id, name: request.victim_name, phone: request.victim_phone },
            location: incidentLocation, severity,
    ...
    try {
        const [rows] = await pool.query(`
            SELECT
                ...
                i.Note AS incident_note, i.Location AS incident_location,
                i.Verification_status, i.Time AS incident_time, i.Status AS incident_status,
                r.Victim_id, r.Request_time, r.Type AS request_type, r.Location AS request_location,
                u.Name AS victim_name, u.Phone_no AS victim_phone,
                a.Assignment_id, a.Status AS assignment_status, a.Assigned_time, a.Completion_time,
                ps.Centre_id AS police_centre, ps.Location AS police_location,
                amb.Ambulance_id, amb.Relevant_hospital, amb.Location AS ambulance_location,
                lc.Law_case_id, lc.Case_type AS law_case_type,
                fs.follow_up_id, fs.Referred_centre,
            ...
            GROUP BY i.Incident_id, i.Request_id, i.Emergency_type, i.Severity,
    ...
    try {
        const [incidents] = await pool.query(`
            SELECT
                ...
                i.Note AS incident_note, i.Location AS incident_location,
                i.Verification_status, i.Time AS incident_time, i.Status AS incident_status,
                r.Victim_id, u.Name AS victim_name, u.Phone_no AS victim_phone,
                a.Assignment_id, a.Status AS assignment_status, a.Assigned_time, a.Completion_time,
                ps.Centre_id AS police_centre, ps.Location AS police_location,
                amb.Ambulance_id, amb.Relevant_hospital, amb.Location AS ambulance_location,
                GROUP_CONCAT(DISTINCT CONCAT(vol_u.Name, ' (', vol.Centre_id, ')')) AS volunteers_assigned
            ...
            GROUP BY i.Incident_id, i.Request_id, i.Emergency_type, i.Severity,
                ...
                a.Assignment_id, a.Status, a.Assigned_time, a.Completion_time,
                ps.Centre_id, ps.Location, amb.Ambulance_id, amb.Relevant_hospital, amb.Location
...
router.put('/incident/:id', lockRow('Incident', 'id'), async (req, res) => {
    ...
    try {
        ...
        const updates = [], params = [];
        if (status) {
            if (!VALID_STATUS.includes(status)) throw new Error(`Invalid status. Use: ${VALID_STATUS.join(', ')}`);
            updates.push('Status = ?'); params.push(status);
        }
        if (verification_status) {
            if (!VALID_VERIFICATION_STATUS.includes(verification_status)) throw new Error('Invalid verification_status.');
        ...
        if (updates.length) {
            ...
            await req.dbConnection.query(`UPDATE Incident SET ${updates.join(', ')} WHERE Incident_id = ?`, params);
        }
...
router.get('/resources/available', async (req, res) => {
    try { res.json(await getAvailableResources()); }
...
router.post('/assignment', async (req, res) => {
    ...
    try {
        ...
        const [existing] = await connection.query('SELECT Assignment_id FROM Assignment WHERE Incident_id = ?', [incident_id]);
        ...
        res.json({ message: 'Assignment created successfully.', assignment_id: assignmentId, assigned_volunteers: assignedVolunteers });
    } catch (err) {
        await connection.query('ROLLBACK');
...
router.put('/assignment/:id', lockRow('Assignment', 'id'), async (req, res) => {
    ...
    try {
        ...
        await req.dbConnection.query('UPDATE Assignment SET Status = ? WHERE Assignment_id = ?', [status, assignmentId]);
...
// CHANGED: removed Status filters on police/ambulance queries and removed
//          status UPDATE statements since those columns no longer exist
router.post('/volunteer/:id/backup-request', async (req, res) => {
    ...
    try {
        ...
        const [volunteer] = await connection.query(`
            SELECT v.*, va.Assignment_id, a.Incident_id, a.Status AS assignment_status,
                i.Emergency_type, i.Severity AS incident_severity, i.Location, i.Note AS incident_note,
                u.Name AS volunteer_name, u.Phone_no AS volunteer_phone,
                vic.User_id AS victim_id, vic_u.Name AS victim_name, vic_u.Phone_no AS victim_phone,
                c.Location AS centre_location
            FROM Volunteer v
            JOIN User u                   ON v.User_id        = u.User_id
            JOIN Centre c                 ON v.Centre_id      = c.Centre_id
            ...
            WHERE v.User_id = ? AND a.Status = 'Ongoing' AND r.Dispatcher_id = ?
        ...
        // CHANGED: no WHERE Status filter since Police_services has no Status column
        const [availPolice]    = await connection.query(`SELECT Centre_id, Location, Centre_number FROM Police_services`);
        // CHANGED: no WHERE Status filter since Ambulance_service has no Status column
        const [availAmbulance] = await connection.query(`SELECT Ambulance_id, Relevant_hospital, Location, Contact_info FROM Ambulance_service`);

        let assignedPolice = null, assignedAmbulance = null;
        const resourceMessages = [];

        if (policeReq && availPolice.length) {
            assignedPolice = availPolice[0].Centre_id;
            // CHANGED: no status update on Police_services
        ...
        if (ambulanceReq && availAmbulance.length) {
            assignedAmbulance = availAmbulance[0].Ambulance_id;
            // CHANGED: no status update on Ambulance_service
...
router.get('/incident/:id/backup-status', async (req, res) => {
    try {
        const [incident] = await pool.query(`
            SELECT i.Incident_id, i.Note, i.Location AS incident_location, i.Emergency_type, i.Severity,
                a.Assignment_id, a.Police_id, a.Ambulance_id,
                ps.Location AS police_location,
                amb.Relevant_hospital, amb.Location AS ambulance_location,
            ...
            WHERE i.Incident_id = ? AND r.Dispatcher_id = ?
            GROUP BY i.Incident_id, i.Note, i.Location, i.Emergency_type, i.Severity,
                a.Assignment_id, a.Police_id, a.Ambulance_id,
                ps.Location, amb.Relevant_hospital, amb.Location
        `, [req.params.id, req.user.id]);

        if (!incident.length) return res.status(404).json({ error: 'Incident not found or not assigned to you.' });

        const notes = incident[0].Note || '';
        const backupHistory = notes.split(/\n| \| /).filter(l => l.includes('BACKUP REQUESTED')).map(l => {
            const withTs    = l.match(/\[(.*?)\] BACKUP REQUESTED by (?:Volunteer )?(.+?): (.*)/);
            const withoutTs = l.match(/BACKUP REQUESTED by (.+?)(?:: (.*))?$/);
            if (withTs)    return { timestamp: withTs[1],  volunteer: withTs[2],    details: withTs[3] || '' };
            if (withoutTs) return { timestamp: null,       volunteer: withoutTs[1], details: withoutTs[2] || '' };
            return null;
        }).filter(Boolean);

        res.json({
            incident: { id: incident[0].Incident_id, type: incident[0].Emergency_type, severity: incident[0].Severity, location: incident[0].Location },
            assignment: {
                id: incident[0].Assignment_id,
                police: incident[0].Police_id ? { id: incident[0].Police_id, location: incident[0].police_location } : null,
                ambulance: incident[0].Ambulance_id ? { id: incident[0].Ambulance_id, hospital: incident[0].Relevant_hospital, location: incident[0].ambulance_location } : null,
                volunteers: incident[0].volunteers_assigned ? incident[0].volunteers_assigned.split(',') : []
            },
...
// CHANGED: removed Status from INSERT and removed status-based WHERE/UPDATE throughout
...
router.post('/lawcase', async (req, res) => {
    ...
    try {
        await connection.query('START TRANSACTION');
        if (!incident_id) throw new Error('incident_id is required.');
        if (!VALID_CASE_TYPES.includes(case_type)) throw new Error(`Invalid case_type. Use: ${VALID_CASE_TYPES.join(', ')}`);
        ...
        const [existing] = await connection.query('SELECT Law_case_id FROM Law_case WHERE Incident_id = ?', [incident_id]);
        ...
        // CHANGED: no Status column in Law_case
        await connection.query(
...
// CHANGED: Law_case has no Status — this route now only updates Case_type / Lawfirm_name
router.put('/lawcase/:id', lockRow('Law_case', 'id'), async (req, res) => {
    const { case_type, lawfirm_name } = req.body;
    ...
    try {
        ...
        if (case_type) {
            if (!VALID_CASE_TYPES.includes(case_type)) throw new Error(`Invalid case_type. Use: ${VALID_CASE_TYPES.join(', ')}`);
            updates.push('Case_type = ?'); params.push(case_type);
        }
        if (lawfirm_name) { updates.push('Lawfirm_name = ?'); params.push(lawfirm_name); }
        if (!updates.length) throw new Error('No valid fields to update (Case_type or Lawfirm_name).');

        params.push(lawCaseId);
        await req.dbConnection.query(`UPDATE Law_case SET ${updates.join(', ')} WHERE Law_case_id = ?`, params);
...
router.get('/lawcases', async (req, res) => {
    try {
        // CHANGED: removed lc.Status from SELECT
        const [cases] = await pool.query(`
            SELECT lc.Law_case_id, lc.Incident_id, lc.Lawfirm_name, lc.Case_type,
                i.Emergency_type, i.Severity, r.Victim_id, u.Name AS victim_name
            FROM Law_case lc
            JOIN Incident i    ON lc.Incident_id = i.Incident_id
            JOIN Request_log r ON i.Request_id   = r.Request_id
            JOIN Victim v      ON r.Victim_id    = v.User_id
...
// CHANGED: removed Status from INSERT and removed status-based WHERE/UPDATE

router.post('/followup', async (req, res) => {
    const { assignment_id, case_type } = req.body;
    let { referred_centre } = req.body;
    const connection = await pool.getConnection();
    let locked = false;
    ...
    try {
        ...
        if (!referred_centre || !referred_centre.trim()) {
            const [[prevCentre]] = await connection.query(
                `SELECT Referred_centre FROM Follow_up_support ORDER BY follow_up_id DESC LIMIT 1`
            );
            referred_centre = prevCentre ? prevCentre.Referred_centre : 'Welfare Crisis Support Centre';
            logger.info(`FOLLOWUP_AUTO_CENTRE - Auto-assigned: ${referred_centre}`);
        }
        ...
        const [existing] = await connection.query('SELECT follow_up_id FROM Follow_up_support WHERE Assignment_id = ?', [assignment_id]);
        ...
        // CHANGED: no Status column in Follow_up_support
        await connection.query(
...
// CHANGED: Follow_up_support has no Status — route now updates Referred_centre / Case_type only
router.put('/followup/:id', lockRow('Follow_up_support', 'id'), async (req, res) => {
    const { case_type, referred_centre } = req.body;
    ...
    try {
        ...
        if (referred_centre) { updates.push('Referred_centre = ?'); params.push(referred_centre); }
        if (!updates.length) throw new Error('No valid fields to update (Case_type or Referred_centre).');

        params.push(followUpId);
        await req.dbConnection.query(`UPDATE Follow_up_support SET ${updates.join(', ')} WHERE follow_up_id = ?`, params);
...
router.get('/followups', async (req, res) => {
    try {
        // CHANGED: removed fs.Status from SELECT
        const [followups] = await pool.query(`
            SELECT fs.follow_up_id, fs.Assignment_id, fs.Referred_centre, fs.Case_type,
                i.Emergency_type, r.Victim_id, u.Name AS victim_name
...
router.post('/incident/:id/admin-takeover', allowRoles('Admin'), async (req, res) => {
    ...
    try {
        ...
        const [[{ acquired }]] = await connection.query(`SELECT GET_LOCK(?, ?) AS acquired`, [`Incident_${incidentId}`, LOCK_TIMEOUT]);

        if (!acquired) return res.status(423).json({ error: 'Could not acquire lock after force-release. Try again.' });

        logger.info(`ADMIN_TAKEOVER - Admin: ${req.user.id} took over Incident: ${incidentId} from Dispatcher: ${incident.dispatcher_name} (${incident.Dispatcher_id})`);
        res.json({
...
router.get('/lawfirms', async (req, res) => {
    try {
        const [rows] = await pool.query(`
            SELECT DISTINCT Lawfirm_name FROM Law_case lc
            JOIN Incident i    ON lc.Incident_id = i.Incident_id
            JOIN Request_log r ON i.Request_id   = r.Request_id
            WHERE r.Dispatcher_id = ?
            ORDER BY Lawfirm_name ASC
        `, [req.user.id]);
        res.json(rows.map(r => r.Lawfirm_name));
    } catch (err) {
        logger.error(`GET_LAWFIRMS_FAILED: ${err.message}`);
        res.status(500).json({ error: err.message });
    }
});
...
router.get('/referred-centres', async (req, res) => {
    try {
        const [rows] = await pool.query(`
            SELECT DISTINCT fs.Referred_centre FROM Follow_up_support fs
            JOIN Assignment a  ON fs.Assignment_id = a.Assignment_id
            JOIN Incident i    ON a.Incident_id    = i.Incident_id
            JOIN Request_log r ON i.Request_id     = r.Request_id
            WHERE r.Dispatcher_id = ?
            ORDER BY fs.Referred_centre ASC
        `, [req.user.id]);
        res.json(rows.map(r => r.Referred_centre));
    } catch (err) {
        logger.error(`GET_REFERRED_CENTRES_FAILED: ${err.message}`);
        res.status(500).json({ error: err.message });
    }
});
...
router.get('/volunteers', async (req, res) => {
    try {
        const centreId = await getDispatcherCentre(req.user.id);
        if (!centreId) return res.status(400).json({ error: 'Dispatcher centre not found.' });

        const [rows] = await pool.query(`
            SELECT v.User_id, u.Name, u.Phone_no, u.Email, v.Status, v.Availability,
                v.Emergency_contact, v.Centre_id, c.Location AS Centre_Location,
            ...
            WHERE v.Centre_id = ?
            GROUP BY v.User_id, u.Name, u.Phone_no, u.Email, v.Status, v.Availability,
                v.Emergency_contact, v.Centre_id, c.Location
            ORDER BY v.Availability DESC, u.Name ASC
        `, [centreId]);

        res.json(rows);
    } catch (err) {
        logger.error(`GET_ALL_VOLUNTEERS_FAILED: ${err.message}`);
        res.status(500).json({ error: err.message });
    }
});

// CHANGED: removed Status column from SELECT on both police and ambulance
router.get('/resources', async (req, res) => {
    try {
        const [police] = await pool.query(`SELECT Centre_id, Location, Centre_number FROM Police_services ORDER BY Location ASC`);
        const [ambulance] = await pool.query(`SELECT Ambulance_id, Relevant_hospital, Location, Contact_info FROM Ambulance_service ORDER BY Location ASC`);
        res.json({ police, ambulance });
    } catch (err) {
        logger.error(`GET_ALL_RESOURCES_FAILED: ${err.message}`);
        res.status(500).json({ error: err.message });
    }
});
...
router.get('/backups', async (req, res) => {
    try {
        // CHANGED: removed ps.Status and amb.Status from SELECT
        const [rows] = await pool.query(`
            SELECT i.Incident_id, i.Note, i.Emergency_type, i.Severity,
                i.Location AS incident_location, i.Status AS incident_status,
                r.Victim_id, u.Name AS victim_name,
            ...
            LEFT JOIN Ambulance_service amb ON a.Ambulance_id = amb.Ambulance_id
            WHERE r.Dispatcher_id = ?
              AND i.Note LIKE '%BACKUP REQUESTED%'
            ORDER BY CASE i.Status WHEN 'Ongoing' THEN 0 WHEN 'Pending' THEN 1 ELSE 2 END, i.Time DESC
        `, [req.user.id]);

        const result = rows.map(row => {
            const notes = row.Note || '';
            const lines = notes.split(/\n| \| /).filter(l => l.includes('BACKUP REQUESTED'));
            const backupEntries = lines.map(l => {
                const withTs    = l.match(/\[(.*?)\] BACKUP REQUESTED by (?:Volunteer )?(.+?): (.*)/);
                const withoutTs = l.match(/BACKUP REQUESTED by (.+?)(?:: (.*))?$/);
                if (withTs)    return { timestamp: withTs[1],  volunteer: withTs[2],    details: withTs[3] || '' };
                if (withoutTs) return { timestamp: null,       volunteer: withoutTs[1], details: withoutTs[2] || '' };
            ...
            return { ...row, backup_requests: backupEntries };
        });

        res.json(result);
    } catch (err) {
        logger.error(`GET_BACKUPS_FAILED: ${err.message}`);
        res.status(500).json({ error: err.message });
    }
});
...
router.get('/centres', async (req, res) => {
    try {
        const [rows] = await pool.query(`SELECT Centre_id, Location, Centre_number FROM Centre ORDER BY Location ASC`);
        res.json(rows);
    } catch (err) {
        logger.error(`GET_CENTRES_FAILED: ${err.message}`);
        res.status(500).json({ error: err.message });
    }
});

router.get('/incident/:id/volunteers', async (req, res) => {
    try {
        const [rows] = await pool.query(`
            SELECT va.Vol_assignment_id, va.Volunteer_id, vol.Status AS volunteer_status,
                vol.Availability, u.Name AS volunteer_name, u.Phone_no AS volunteer_phone, vol.Centre_id
            FROM Volunteer_assignment va
            JOIN Assignment a  ON va.Assignment_id = a.Assignment_id
            JOIN Incident i    ON a.Incident_id    = i.Incident_id
            JOIN Request_log r ON i.Request_id     = r.Request_id
            JOIN Volunteer vol ON va.Volunteer_id  = vol.User_id
            JOIN User u        ON vol.User_id      = u.User_id
            WHERE i.Incident_id = ? AND r.Dispatcher_id = ?
        `, [req.params.id, req.user.id]);
        res.json(rows);
    } catch (err) {
        logger.error(`GET_INCIDENT_VOLUNTEERS_FAILED: ${err.message}`);
        res.status(500).json({ error: err.message });
    }
});

router.delete('/volunteer-assignment/:id', async (req, res) => {
    const volAssignmentId = req.params.id;
    const connection = await pool.getConnection();
    ...
    try {
        ...
        const [[row]] = await connection.query(`
            SELECT va.Vol_assignment_id, va.Volunteer_id, va.Assignment_id,
                a.Status AS assignment_status, r.Dispatcher_id
            FROM Volunteer_assignment va
            JOIN Assignment a    ON va.Assignment_id = a.Assignment_id
            JOIN Incident i      ON a.Incident_id    = i.Incident_id
            JOIN Request_log r   ON i.Request_id     = r.Request_id
            WHERE va.Vol_assignment_id = ?
        `, [volAssignmentId]);

        if (!row) { await connection.query('ROLLBACK'); return res.status(404).json({ error: 'Volunteer assignment not found.' }); }
        if (row.Dispatcher_id !== req.user.id) { await connection.query('ROLLBACK'); return res.status(403).json({ error: 'Access denied. This assignment does not belong to you.' }); }
        if (row.assignment_status === 'Ongoing' || row.assignment_status === 'Completed') {
            await connection.query('ROLLBACK');
            return res.status(400).json({ error: `Cannot remove volunteer — assignment is already ${row.assignment_status}. Volunteers can only be removed while the assignment is Pending.` });
        }

        await connection.query(`DELETE FROM Volunteer_assignment WHERE Vol_assignment_id = ?`, [volAssignmentId]);
        await connection.query(`UPDATE Volunteer SET Availability = 'Yes', Status = NULL WHERE User_id = ?`, [row.Volunteer_id]);

        await connection.query('COMMIT');
        logger.info(`VOLUNTEER_REMOVED - Vol_assignment: ${volAssignmentId}, Volunteer: ${row.Volunteer_id}, Dispatcher: ${req.user.id}`);
        res.json({ message: 'Volunteer removed from assignment and availability reset to Yes.', volunteer_id: row.Volunteer_id });
    } catch (err) {
        await connection.query('ROLLBACK');
        logger.error(`VOLUNTEER_REMOVE_FAILED: ${err.message}`);
        res.status(500).json({ error: err.message });
    } finally {
        connection.release();
    }
