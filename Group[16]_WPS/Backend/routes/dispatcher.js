...
// CHANGED: removed Centre_id filter so all available volunteers across all centres are returned
const getAvailableVolunteers = async (severity = null) => {
    let query = `
        SELECT
            v.User_id,
            u.Name,
...
// CHANGED: removed centre check — dispatchers can now assign volunteers from any centre
const assignVolunteers = async (connection, assignmentId, volunteerIds, dispatcherId) => {
    const assigned = [];

    for (const volunteerId of volunteerIds) {
        // CHANGED: removed Centre_id check against dispatcher's centre
    ...
    try {
        const [workload] = await pool.query(`
            SELECT
                ...
                -- Active requests: unprocessed calls, False-typed excluded
                (
                    SELECT COUNT(*)
                    FROM Request_log r
        ...
        const [history] = await pool.query(`
            SELECT
                -- Total handled = completed incidents + false requests
                (
                    SELECT COUNT(DISTINCT r2.Request_id)
                    FROM Request_log r2
                    LEFT JOIN Incident i2 ON r2.Request_id = i2.Request_id
                    WHERE r2.Dispatcher_id = ?
                      AND (i2.Status = 'Completed' OR r2.Type = 'False')
                ) AS total_handled,
...
// CHANGED: now returns all available volunteers across all centres (no centre filter)
router.get('/volunteers/available', async (req, res) => {
    try {
        const { severity } = req.query;
        const volunteers = await getAvailableVolunteers(severity || null);
        res.json(volunteers);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
// Law_case has no Status — only updates Case_type / Lawfirm_name
...
// Follow_up_support has no Status — only updates Referred_centre / Case_type
...
// CHANGED: fetches all volunteers across all centres (not just dispatcher's centre)
...
// CHANGED: returns volunteers for an incident regardless of their centre
    ...
    try {
        const [rows] = await pool.query(`
            SELECT va.Vol_assignment_id, va.Volunteer_id, vol.Status AS volunteer_status,
                vol.Availability, u.Name AS volunteer_name, u.Phone_no AS volunteer_phone,
                vol.Centre_id, c.Location AS centre_location
