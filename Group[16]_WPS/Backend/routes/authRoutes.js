...
// ✏️ CHANGE: new logout endpoint
// For admins: locks their Admin row, checks for pending recovery requests,
// blocks logout if any exist, otherwise sets Availability to No and commits.
// JWT is stateless so token expiry still governs actual session — this just
// sets admin offline so no new recovery requests get assigned to them.
router.post('/logout', verifyJWT, async (req, res) => {
    ...
    try {
        ...
        if (adminRows.length) {
            ...
            // ✏️ CHANGE: FOR UPDATE locks the Admin row so any concurrent recovery
            // assignment targeting this admin blocks until this transaction completes
            await conn.query(
                `SELECT User_id FROM Admin WHERE User_id = ? FOR UPDATE`,
                [req.user.id]
            );

            // Block logout if there are still pending requests assigned to this admin
            ...
            // ✏️ CHANGE: set Availability to No on logout
            await conn.query(
                `UPDATE Admin SET Availability = 'No' WHERE User_id = ?`, [req.user.id]
            );
            await conn.query('COMMIT');
