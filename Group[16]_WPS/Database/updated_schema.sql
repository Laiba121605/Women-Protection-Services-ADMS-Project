...
CREATE TABLE Admin (
    User_id      VARCHAR(20) PRIMARY KEY,
    Availability VARCHAR(3) CHECK (Availability IN ('Yes', 'No')) DEFAULT 'No', -- ✏️ CHANGE: added for round-robin filtering and logout control
    FOREIGN KEY (User_id) REFERENCES User(User_id)
);
...
SELECT 
    ...
    r.Location          AS Request_Location,   -- ← ADDED
    r.Dispatcher_id,
    u_disp.Name         AS Dispatcher_Name,
...
LEFT JOIN Dispatcher d          ON r.Dispatcher_id  = d.User_id
LEFT JOIN User u_disp           ON d.User_id        = u_disp.User_id
