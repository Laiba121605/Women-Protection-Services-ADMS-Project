...
//
// ── CHANGES ──────────────────────────────────────────────────────────────────
// 1. Pipeline reduced from 6 stages to 4 stages:
//      Request Submitted → Incident Verified → Response Dispatched → Case Closed
//    "Services Assigned" and "Support Ongoing" stages REMOVED from pipeline.
// 2. "Response Dispatched" stage now ticks (fills green) as soon as Assignment_id
//    exists — previously it required Police_Centre or Ambulance_id to be set.
// ─────────────────────────────────────────────────────────────────────────────
...
const VictimDashboard = ({ embeddedMode = false, onLogout }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const res = await API.post('/victim/request', { Note: note.trim(), Location: location.trim() });
  ...
  const IncidentCard = ({ item }) => {
    ...
    // Pipeline has 4 dots at indices 0,1,2,3.
    // A dot is GREEN (done) when i < stage, PURPLE (current) when i === stage, GREY when i > stage.
    // We want "Response Dispatched" (index 2) to be GREEN as soon as Assignment_id exists,
    // so stage must be > 2, i.e. stage=3 when assignment exists (not Completed).
    // stage=4 means all 4 dots are green (Completed).
    const stage = item.Incident_Status === 'Completed' ? 4  // all 4 dots green
      : item.Assignment_id                              ? 3  // dots 0,1,2 green (Response Dispatched ticked)
      : item.Incident_id                                ? 1  // dot 0 green, dot 1 current
      : 0;                                                   // dot 0 current (just submitted)

    // CHANGE: Only 4 stages in pipeline (removed "Services Assigned" and "Support Ongoing")
    const stages = [
        ...
        <div style={V.content}>
          {incidents.length > 0 && (
            <div style={V.filterBar}>
              ...
              <div style={V.filterField}>
                ...
                <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} style={V.filterInput} />
              </div>
              <div style={V.filterField}>
                <span style={V.filterIconLabel}>📅 To</span>
                <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} style={V.filterInput} />
              </div>
              {hasActiveFilters && (
          ...
          {loading ? (
            ...
            <div style={V.empty}>
              ...
              <button onClick={() => setTab('submit')} style={V.emptyAction}>Submit an Emergency Request</button>
            </div>
          ...
          <div style={V.formCard}>
            <div style={V.formHeader}>
              ...
              <div>
                ...
                <p style={V.formHeaderSub}>A dispatcher will be notified immediately and assigned to your case.</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} style={V.form} autoComplete="off">
              ...
              <button type="submit" disabled={submitting} style={submitting ? V.submitBtnDisabled : V.submitBtn}>
                {submitting ? (
                  <span style={V.btnContent}><span style={V.spinner2} /> Submitting…</span>
                ) : (
...
const V = {
  page: { minHeight: '100vh', backgroundColor: '#fdf5f5', fontFamily: "'Segoe UI', system-ui, sans-serif", paddingBottom: '60px' },
  embeddedWrapper: { fontFamily: "'Segoe UI', system-ui, sans-serif", paddingBottom: '20px' },
  ...
  avatar: { width: '52px', height: '52px', borderRadius: '50%', backgroundColor: '#fdf2f8', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px' },
  ...
  errorBanner: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#fff5f5', color: '#c0392b', padding: '12px 32px', fontSize: '13px', borderBottom: '1px solid #fecaca' },
  errorClose: { background: 'none', border: 'none', color: '#c0392b', cursor: 'pointer', fontSize: '14px' },
  tabs: { display: 'flex', gap: '4px', padding: '20px 32px 0', borderBottom: '2px solid #eee', backgroundColor: '#fff', alignItems: 'flex-end' },
  tab: { padding: '10px 20px', border: 'none', background: 'none', fontSize: '14px', fontWeight: '600', color: '#999', cursor: 'pointer', borderBottom: '2px solid transparent', marginBottom: '-2px', borderRadius: '6px 6px 0 0' },
  tabActiveIncidents: { color: '#7b2d8b', borderBottom: '2px solid #7b2d8b', backgroundColor: '#fdf5ff' },
  tabActiveSubmit: { color: '#c0392b', borderBottom: '2px solid #c0392b', backgroundColor: '#fff5f5' },
  content: { padding: '24px 32px' },
  filterBar: { display: 'flex', flexWrap: 'wrap', gap: '10px', marginBottom: '16px', alignItems: 'center', padding: '14px 16px', backgroundColor: '#fff', borderRadius: '12px', border: '1px solid #eef0f3', boxShadow: '0 1px 6px rgba(0,0,0,0.04)' },
  filterField: { display: 'flex', alignItems: 'center', gap: '6px', backgroundColor: '#f7f9fc', borderRadius: '8px', border: '1.5px solid #e8eaed', padding: '0 10px', flex: '1 1 160px', minWidth: '140px' },
  ...
  filterInput: { border: 'none', outline: 'none', backgroundColor: 'transparent', fontSize: '13px', color: '#333', padding: '9px 0', width: '100%', fontFamily: 'inherit' },
  ...
  pollingBadge: { fontSize: '11px', color: '#27ae60', fontWeight: '600', marginLeft: '4px', marginBottom: '4px', userSelect: 'none' },
  ...
  emptyAction: { marginTop: '8px', padding: '10px 24px', borderRadius: '8px', border: 'none', backgroundColor: '#c0392b', color: '#fff', fontSize: '13px', fontWeight: '700', cursor: 'pointer' },
  spinner: { width: '32px', height: '32px', border: '3px solid #f5e6fb', borderTop: '3px solid #7b2d8b', borderRadius: '50%', animation: 'spin 0.8s linear infinite' },
  pipeline: { display: 'flex', alignItems: 'flex-start', padding: '16px 20px', backgroundColor: '#fafbfc', borderBottom: '1px solid #f0f0f0', overflowX: 'auto', cursor: 'default' },
  pipelineStep: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px', minWidth: '64px' },
  pipelineDot: { width: '36px', height: '36px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background-color 0.2s', cursor: 'default' },
  pipelineLabel: { fontSize: '10px', textAlign: 'center', lineHeight: '1.3', margin: 0, userSelect: 'none' },
  pipelineLine: { flex: 1, height: '2px', marginTop: '17px', minWidth: '16px', transition: 'background-color 0.2s' },
  pendingNote: { fontSize: '12px', color: '#aaa', margin: '4px 0 0', fontStyle: 'italic', cursor: 'default', userSelect: 'none' },
  card: { backgroundColor: '#fff', borderRadius: '14px', border: '1px solid #eef0f3', boxShadow: '0 2px 12px rgba(0,0,0,0.05)', overflow: 'hidden' },
  cardHeader: { display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px', padding: '16px 20px', backgroundColor: '#fafbfc', borderBottom: '1px solid #f0f0f0', cursor: 'pointer', userSelect: 'none' },
  ...
  sectionTitle: { margin: '0 0 8px', fontSize: '11px', fontWeight: '700', color: '#bbb', textTransform: 'uppercase', letterSpacing: '1px', cursor: 'default', userSelect: 'none' },
  ...
  formCard: { maxWidth: '600px', backgroundColor: '#fff', borderRadius: '16px', border: '1px solid #eef0f3', boxShadow: '0 2px 16px rgba(0,0,0,0.06)', overflow: 'hidden' },
  formHeader: { display: 'flex', alignItems: 'flex-start', gap: '14px', padding: '20px 24px', backgroundColor: '#fff5f5', borderBottom: '1px solid #fee2e2' },
  ...
  formError: { display: 'flex', alignItems: 'center', gap: '8px', backgroundColor: '#fff5f5', color: '#c0392b', padding: '12px 24px', fontSize: '13px', borderBottom: '1px solid #fecaca' },
  formErrorDot: { fontSize: '8px' },
  form: { padding: '24px', display: 'flex', flexDirection: 'column', gap: '20px' },
  fieldGroup: { display: 'flex', flexDirection: 'column', gap: '6px' },
  label: { fontSize: '13px', fontWeight: '600', color: '#555' },
  textarea: { padding: '12px 14px', borderRadius: '10px', border: '1.5px solid #e0e0e0', outline: 'none', fontSize: '13px', color: '#333', resize: 'vertical', fontFamily: 'inherit', backgroundColor: '#fafafa', lineHeight: '1.5' },
  input: { padding: '12px 14px', borderRadius: '10px', border: '1.5px solid #e0e0e0', outline: 'none', fontSize: '13px', color: '#333', backgroundColor: '#fafafa' },
  charCount: { margin: '4px 0 0', fontSize: '11px', color: '#bbb', textAlign: 'right' },
  submitBtn: { padding: '13px', borderRadius: '10px', border: 'none', background: 'linear-gradient(135deg, #c0392b, #e74c3c)', color: '#fff', fontSize: '15px', fontWeight: '700', cursor: 'pointer', boxShadow: '0 4px 15px rgba(192,57,43,0.3)' },
  submitBtnDisabled: { padding: '13px', borderRadius: '10px', border: 'none', backgroundColor: '#f5b7b1', color: '#fff', fontSize: '15px', fontWeight: '700', cursor: 'not-allowed' },
  btnContent: { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' },
  spinner2: { width: '14px', height: '14px', border: '2px solid rgba(255,255,255,0.4)', borderTop: '2px solid #fff', borderRadius: '50%', display: 'inline-block', animation: 'spin 0.8s linear infinite' },
  warningBox: { margin: '0 24px 24px', padding: '12px 16px', borderRadius: '10px', backgroundColor: '#fffbf0', border: '1px solid #f9e4b7' },
