    ...
    <div style={D.page}>
      ...
      <div style={D.header}>
        ...
        <div style={D.headerRight}>
          {/* Dispatcher can register THEMSELVES as a victim — but cannot create victim accounts */}
      ...
      <div style={D.content}>
        ...
        {tab === 'calls'      && <CallsTab      showToast={showToast} />}
...
// ══════════════════════════════════════════════════════════════════════════════
// CALLS TAB
// Dispatchers can search for existing victims and log calls.
// They CANNOT create new victim accounts — that is Admin's responsibility.
// ══════════════════════════════════════════════════════════════════════════════
const CallsTab = ({ showToast }) => {
  ...
  const loadCalls = useCallback(async () => {
    setLoading(true);
    try { const res = await API.get('/dispatcher/calls'); setCalls(res.data); }
  ...
  const filteredCalls = calls.filter(c => {
    ...
    if (searchCall.trim() &&
        !c.Request_id?.toLowerCase().includes(searchCall.toLowerCase()) &&
        !c.victim_name?.toLowerCase().includes(searchCall.toLowerCase()))
      return false;
    return true;
  });
  ...
  const saveEdit = async () => {
    setEditErr('');
    if (!editNote.trim() && !editType) return setEditErr('Nothing to update — enter a new note or change the type');
    setSavingEdit(true);
    try {
  ...
  // ── New Call view — search only, no create ──
  if (view === 'new') return (
    <div>
      <button onClick={() => {
      ...
      <div style={D.formCard}>
        ...
        <p style={{ fontSize:'12px', color:'#888', margin:'0 0 10px' }}>
          Search for an existing registered victim. To register a new victim account, contact an Admin.
        </p>
        ...
        {/* Found — but not a registered victim */}
        {victimResult?.found && (() => {
          const u = victimResult.user;
          const isVictim = u.is_victim === 1 || u.is_victim === true || victimResult.confirmed;
          if (!isVictim) {
            return (
              <div style={D.warnBox}>
                ...
                <p style={{ margin:'0 0 8px', fontSize:'13px' }}>
                  <strong>{u.Name}</strong> — {u.Phone_no} (ID: {u.User_id})
                </p>
                <p style={{ margin:0, fontSize:'12px', color:'#888' }}>
                  Please ask an Admin to register this person as a victim, then search again.
                </p>
              </div>
            );
          }
          return (
            <div style={D.successBox}>
              ✅ Victim found: <strong>{u.Name}</strong> — {u.Phone_no} (ID: {u.User_id})
            </div>
          );
        })()}
        ...
        {victimResult && !victimResult.found && (
          <div style={D.infoBox}>
            <p style={{ margin:0, fontSize:'13px', color:'#555' }}>
              ...
              Victim accounts must be created by an Admin — please contact one to register this person.
            </p>
          </div>
        )}
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
          ...
          <input    value={callLocation} onChange={e=>setCallLocation(e.target.value)} placeholder="Location (be specific — e.g. 14 Gulberg III, Lahore) *" style={D.input} />
          ...
          <button
            ...
            disabled={
              loggingCall ||
              !victimResult?.user ||
              !(victimResult.user.is_victim === 1 || victimResult.user.is_victim === true || victimResult.confirmed)
            }
            style={D.btnPrimary}
          >
  ...
  return (
    <div>
      <div style={D.filterBar}>
        <input value={searchCall} onChange={e=>setSearchCall(e.target.value)} placeholder="Search by Request ID or victim name" style={{ ...D.input, flex:1 }} />
        ...
        {(searchCall || filterStatus) && <button onClick={()=>{setSearchCall('');setFilterStatus('');}} style={D.clearBtn}>✕ Clear</button>}
      </div>
        ...
        : filteredCalls.map(c => (
          <div key={c.Request_id} style={D.listRow}>
            <div style={D.listRowLeft}>
              ...
              <p style={D.listSub}>{c.victim_name} · {c.victim_phone} · Logged: {fmt(c.Request_time)}</p>
...
const IncidentsTab = ({ showToast }) => {
  ...
  if (view === 'detail' && detail) return (
    <div>
      <button onClick={() => { setView('list'); setDetail(null); }} style={D.backBtn}>← Back</button>
      <div style={D.detailCard}>
        <SectionTitle>Incident</SectionTitle>
        ...
        {detail.Assignment_id ? (
          <>
                ...
                {assignedVols.length === 0
                  ...
                  : assignedVols.map(v => (
                    <div key={v.Vol_assignment_id} style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                      <span style={D.fieldValue}>{v.volunteer_name} ({v.Centre_id})</span>
                      {detail.assignment_status === 'Pending' ? (
                        ...
                        <span title={`Cannot remove — assignment is ${detail.assignment_status}`}
                          style={{ fontSize:'11px', color:'#ccc', cursor:'default', userSelect:'none' }}>🔒</span>
                      )}
                    </div>
                  ))
                }
            ...
            {(assignedVols.length === 0 || volunteers.length > 0) && (
              <>
                <div style={D.divider} />
                ...
                <div style={{ display:'flex', flexWrap:'wrap', gap:'6px', marginBottom:'10px' }}>
                  ...
                  {volunteers.length === 0 && <p style={D.pendingNote}>No volunteers available at your centre.</p>}
                </div>
                <Err msg={assignErr} />
                {volunteers.length > 0 && <button onClick={assignMoreVolunteers} disabled={assigning || !selVols.length} style={D.btn}>{assigning?'…':'Assign Selected'}</button>}
              </>
            )}
          </>
        ) : (
          <>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'8px', marginTop:'12px' }}>
              ...
              <p style={{ fontSize:'12px', color:'#888', margin:'4px 0' }}>Available volunteers:</p>
...
// CASES TAB
//
// ROOT CAUSE OF CRASH: The original code called `/admin/referred-centres` and
// `/admin/lawfirms`. These routes do not exist — the correct dispatcher routes
// are `/dispatcher/referred-centres` and `/dispatcher/lawfirms`.
// A 401/403/404 from those requests would cause an unhandled rejection that
// crashed React state and redirected to login.
//
// FIX: Use the correct `/dispatcher/*` paths. Added a graceful error state so
// a failed fetch shows an inline error + Retry button instead of crashing.
// ══════════════════════════════════════════════════════════════════════════════
const CasesTab = ({ showToast }) => {
  ...
  // referredCentres: distinct Referred_centre strings from Follow_up_support (free-text, NOT the Centre table)
  const [referredCentres, setReferredCentres] = useState([]);
  // lawFirms: distinct Lawfirm_name strings from Law_case for this dispatcher
  ...
  const loadAll = useCallback(async () => {
    ...
    setErr('');
    try {
      // ✅ FIXED: Use /dispatcher/* paths — NOT /admin/* paths.
      // The old /admin/referred-centres and /admin/lawfirms routes don't exist
      // and were causing 404/401 errors that crashed the tab.
      const [lRes, fRes, iRes, rcRes, lfRes] = await Promise.all([
        API.get('/dispatcher/lawcases'),
        API.get('/dispatcher/followups'),
        API.get('/dispatcher/incidents'),
        API.get('/dispatcher/referred-centres'),   // ✅ correct path
        API.get('/dispatcher/lawfirms'),            // ✅ correct path
      ]);

      setLawCases(lRes.data || []);
      setFollowups(fRes.data || []);
      setIncidents(iRes.data || []);
      // Both endpoints return a plain string[] — guard with Array.isArray
      setReferredCentres(Array.isArray(rcRes.data) ? rcRes.data : []);
      setLawFirms(Array.isArray(lfRes.data) ? lfRes.data : []);
      setAssignments(
        (iRes.data || [])
          .filter(i => i.Assignment_id)
          .map(i => ({
      ...
      // Show inline error — do NOT let this propagate to crash the whole page
      setErr(e.response?.data?.error || 'Failed to load cases. Please try again.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const createLawCase = async () => {
    ...
    if (lawFirmSel === 'other' && !lawFirmCustom.trim())
      return setLawErr('Please enter a law firm name');
    const resolvedFirm = lawFirmSel === 'other' ? lawFirmCustom.trim()
                       : lawFirmSel             ? lawFirmSel
                       : undefined; // backend defaults to 'Legal Aid Department'
    setCreatingLaw(true);
    try {
      const res = await API.post('/dispatcher/lawcase', {
        incident_id: lawIncident, lawfirm_name: resolvedFirm, case_type: lawCaseType,
      });
  ...
  const createFollowup = async () => {
    ...
    if (fupCentreSel === 'other' && !fupCentreCustom.trim())
      return setFupErr('Please enter a centre name');
    // Empty string → backend auto-assigns from the most recent Follow_up_support entry
    const resolvedCentre = fupCentreSel === 'other' ? fupCentreCustom.trim()
                         : fupCentreSel             ? fupCentreSel
                         : '';
    setCreatingFup(true);
    try {
      const res = await API.post('/dispatcher/followup', {
        assignment_id: fupAssign, referred_centre: resolvedCentre, case_type: fupCaseType,
      });
  ...
  const updateLawCase = async (id, status) => {
    setUpdatingId(id); setUpdateErr('');
    try {
      await API.put(`/dispatcher/lawcase/${id}`, { status });
      showToast(`Law case → ${status}`);
      setManagingLaw(prev => prev ? { ...prev, Status: status } : null);
      await loadAll();
    } catch (e) { setUpdateErr(e.response?.data?.error || 'Update failed'); }
  ...
  const updateFollowup = async (id, status) => {
    setUpdatingId(id); setUpdateErr('');
    try {
      await API.put(`/dispatcher/followup/${id}`, { status });
      showToast(`Follow-up → ${status}`);
      setManagingFup(prev => prev ? { ...prev, Status: status } : null);
      await loadAll();
    } catch (e) { setUpdateErr(e.response?.data?.error || 'Update failed'); }
    finally { setUpdatingId(null); }
  };
  ...
  // Graceful error state — inline, never crashes/redirects
  if (err) return (
  ...
  if (managingLaw) return (
    <div>
      <button onClick={() => { setManagingLaw(null); setUpdateErr(''); }} style={D.backBtn}>← Back</button>
      <div style={D.detailCard}>
        ...
        <Field label="Status"    value={statusBadge(managingLaw.Status)} />
        ...
        <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' }}>
          {STATUSES.map(s => (
            <button key={s} onClick={() => updateLawCase(managingLaw.Law_case_id, s)}
              disabled={updatingId === managingLaw.Law_case_id || managingLaw.Status === s}
              style={{ ...D.btn, opacity: managingLaw.Status === s ? 0.35 : 1, ...(managingLaw.Status === s ? { cursor:'default' } : {}) }}>
              {updatingId === managingLaw.Law_case_id ? '…' : s}
            </button>
          ))}
        </div>
      </div>
  ...
  if (managingFup) return (
    <div>
      <button onClick={() => { setManagingFup(null); setUpdateErr(''); }} style={D.backBtn}>← Back</button>
      <div style={D.detailCard}>
        ...
        <Field label="Status"          value={statusBadge(managingFup.Status)} />
        ...
        <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' }}>
          {STATUSES.map(s => (
            <button key={s} onClick={() => updateFollowup(managingFup.follow_up_id, s)}
              disabled={updatingId === managingFup.follow_up_id || managingFup.Status === s}
              style={{ ...D.btn, opacity: managingFup.Status === s ? 0.35 : 1, ...(managingFup.Status === s ? { cursor:'default' } : {}) }}>
              {updatingId === managingFup.follow_up_id ? '…' : s}
            </button>
          ))}
        </div>
      </div>
    ...
    <div>
      ...
      {subTab === 'law' && (
        <>
          ...
          {showLawForm && (
            <div style={D.formCard}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
                ...
                {/* Incident — exclude those that already have a law case */}
                <select value={lawIncident} onChange={e=>setLawIncident(e.target.value)} style={D.select}>
                  <option value="">— Select Incident * —</option>
                  {incidents
                    .filter(i => !lawCases.find(lc => lc.Incident_id === i.Incident_id))
                    .map(i => (
                      <option key={i.Incident_id} value={i.Incident_id}>
                        {i.Incident_id} — {i.victim_name} ({i.Emergency_type}, {i.Severity})
                      </option>
                    ))
                  }
                </select>

                {/* Law firm — dropdown of names already used in Law_case by this dispatcher */}
                <select
                  value={lawFirmSel}
                  onChange={e => { setLawFirmSel(e.target.value); setLawFirmCustom(''); }}
                  style={D.select}
                >
                ...
                {lawFirmSel === 'other' && (
                  <input
                    value={lawFirmCustom}
                    onChange={e => setLawFirmCustom(e.target.value)}
                    placeholder="Enter law firm name"
                    style={D.input}
                  />
                )}

                <select value={lawCaseType} onChange={e=>setLawCaseType(e.target.value)} style={D.select}>
                  {CASE_TYPES.map(t=><option key={t}>{t}</option>)}
                </select>
                <Err msg={lawErr} />
                <div style={{ display:'flex', gap:'8px' }}>
                  <button onClick={createLawCase} disabled={creatingLaw} style={D.btn}>{creatingLaw?'Creating…':'Create'}</button>
                  <button onClick={()=>{setShowLawForm(false);setLawErr('');setLawFirmSel('');setLawFirmCustom('');}} style={D.btnGhost}>Cancel</button>
                </div>
              </div>
            </div>
          )}
          ...
          {lawCases.length === 0
            ? <Empty icon="⚖️" text="No law cases yet." />
            : lawCases.map(lc => (
              <div key={lc.Law_case_id} style={D.listRow}>
                <div style={D.listRowLeft}>
                  <div style={{ display:'flex', gap:'8px', alignItems:'center', flexWrap:'wrap' }}>
                    <span style={D.listId}>{lc.Law_case_id}</span>{statusBadge(lc.Case_type)}{statusBadge(lc.Status)}
                  </div>
      ...
      {subTab === 'followup' && (
        <>
          ...
          {showFupForm && (
            <div style={D.formCard}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
                ...
                {/* Assignment — exclude those that already have a follow-up */}
                <select value={fupAssign} onChange={e=>setFupAssign(e.target.value)} style={D.select}>
                  <option value="">— Select Assignment * —</option>
                  {assignments
                    .filter(a => !followups.find(f => f.Assignment_id === a.Assignment_id))
                    .map(a => (
                ...
                {/*
                  Referred centre — dropdown of distinct Referred_centre values from
                  Follow_up_support table. This is a free-text varchar field and is NOT
                  linked to the Centre table (which is for operational centres).
                  Leave blank to let the backend auto-assign.
                */}
                <select
                  value={fupCentreSel}
                  onChange={e => { setFupCentreSel(e.target.value); setFupCentreCustom(''); }}
                  style={D.select}
                >
                  <option value="">— Referred centre (blank = auto-assign) —</option>
                  {referredCentres.map(c => (
                ...
                {fupCentreSel === 'other' && (
                  <input
                    value={fupCentreCustom}
                    onChange={e => setFupCentreCustom(e.target.value)}
                    placeholder="e.g. Dastak Welfare Centre, Lahore"
                    style={D.input}
                  />
                )}

                <select value={fupCaseType} onChange={e=>setFupCaseType(e.target.value)} style={D.select}>
                  {CASE_TYPES.map(t=><option key={t}>{t}</option>)}
                </select>
                <Err msg={fupErr} />
                <div style={{ display:'flex', gap:'8px' }}>
                  <button onClick={createFollowup} disabled={creatingFup} style={D.btn}>{creatingFup?'Creating…':'Create'}</button>
                  <button onClick={()=>{setShowFupForm(false);setFupErr('');setFupCentreSel('');setFupCentreCustom('');}} style={D.btnGhost}>Cancel</button>
                </div>
              </div>
            </div>
          )}
          ...
          {followups.length === 0
            ? <Empty icon="🏥" text="No follow-ups yet." />
            : followups.map(f => (
              <div key={f.follow_up_id} style={D.listRow}>
                <div style={D.listRowLeft}>
                  <div style={{ display:'flex', gap:'8px', alignItems:'center', flexWrap:'wrap' }}>
                    <span style={D.listId}>{f.follow_up_id}</span>{statusBadge(f.Case_type)}{statusBadge(f.Status)}
                  </div>
...
const ResourcesTab = () => {
  ...
  const matchLoc = (str) => !searchLoc.trim() ||
    str?.toLowerCase().includes(searchLoc.trim().toLowerCase());

  const police    = (data?.police    || []).filter(p =>
    (!filter || p.Status === filter) && matchLoc(p.Location)
  );
  const ambulance = (data?.ambulance || []).filter(a =>
    (!filter || a.Status === filter) && (matchLoc(a.Location) || matchLoc(a.Relevant_hospital))
  );

  const hasFilters = filter || searchLoc.trim();
    ...
    <div>
      <div style={{ display:'flex', gap:'4px', marginBottom:'12px', borderBottom:'2px solid #eee', alignItems:'flex-end', flexWrap:'wrap' }}>
        ...
        <div style={{ marginLeft:'auto', display:'flex', gap:'8px', marginBottom:'4px', flexWrap:'wrap' }}>
          <input value={searchLoc} onChange={e=>setSearchLoc(e.target.value)} placeholder="Search by location…" style={{ ...D.input, width:'180px' }} />
          <select value={filter} onChange={e=>setFilter(e.target.value)} style={{ ...D.select, width:'140px' }}>
            <option value="">All statuses</option>
            {STATUSES.map(s=><option key={s}>{s}</option>)}
          </select>
          {hasFilters && <button onClick={()=>{setFilter('');setSearchLoc('');}} style={D.clearBtn}>✕ Clear</button>}
        </div>
      </div>

      {subTab === 'police' && (
        police.length === 0 ? <Empty icon="👮" text="No police centres match filter." /> :
        police.map(p => (
          <div key={p.Centre_id} style={D.listRow}>
            <div style={D.listRowLeft}>
              <div style={{ display:'flex', gap:'8px', alignItems:'center' }}>
                <span style={D.listId}>{p.Centre_id}</span>{statusBadge(p.Status)}
              </div>
              <p style={D.listSub}>{p.Location} · {p.Centre_number}</p>
            </div>
          </div>
        ))
      )}
      ...
      {subTab === 'ambulance' && (
        ambulance.length === 0 ? <Empty icon="🚑" text="No ambulances match filter." /> :
        ambulance.map(a => (
          <div key={a.Ambulance_id} style={D.listRow}>
            <div style={D.listRowLeft}>
              <div style={{ display:'flex', gap:'8px', alignItems:'center' }}>
                <span style={D.listId}>{a.Ambulance_id}</span>{statusBadge(a.Status)}
              </div>
              <p style={D.listSub}>{a.Relevant_hospital} · {a.Location} · {a.Contact_info}</p>
            </div>
          </div>
        ))
      )}
...
const VolunteersTab = () => {
  ...
  return (
    <div>
      ...
      <div style={D.listHeader}>
        <p style={D.listCount}>Showing {filtered.length} of {volunteers.length} volunteer{volunteers.length!==1?'s':''}</p>
      </div>
      <Err msg={err} />
      {filtered.length === 0
        ? <Empty icon="🤝" text={volunteers.length===0?'No volunteers at your centre.':'No volunteers match filters.'} />
        : filtered.map(v => (
          <div key={v.User_id} style={D.listRow}>
            <div style={D.listRowLeft}>
              ...
              <p style={D.listSub}>{v.Phone_no} · {v.Email} · {v.total_assignments} assignment{v.total_assignments!==1?'s':''}</p>
            </div>
          </div>
        ))
      }
...
const BackupsTab = ({ showToast }) => {
  ...
  return (
    <div>
      ...
      <Err msg={err} />
      {backups.length === 0
        ? <Empty icon="🆘" text="No active backup requests." />
        : backups.map(b => (
              ...
              <div style={{ marginTop:'12px', borderTop:'1px solid #f5f5f5', paddingTop:'12px' }}>
                ...
                {b.backup_requests.map((br, i) => (
                  <div key={i} style={{ padding:'8px 12px', backgroundColor:'#fff5f5', borderRadius:'8px', marginBottom:'6px', border:'1px solid #fecaca' }}>
                    <p style={{ margin:'0 0 4px', fontSize:'12px', fontWeight:'700', color:'#c0392b' }}>
                      {br.volunteer} · {new Date(br.timestamp).toLocaleString('en-PK', { hour12: true })}
                    </p>
                ...
                <div style={{ marginTop:'10px' }}>
                  ...
                  <Field label="👮 Police"    value={b.police_centre ? `${b.police_centre} (${b.police_status})` : 'Not assigned'} />
                  <Field label="🚑 Ambulance" value={b.Ambulance_id  ? `${b.Ambulance_id} (${b.ambulance_status})`  : 'Not assigned'} />
                </div>
              </div>
            )}
          </div>
