...
// ══════════════════════════════════════════════════════════════════════════════
// CALLS TAB
// FIX: Was calling GET /dispatcher/calls/all which does not exist in the backend.
// Now calls GET /dispatcher/calls which returns only this dispatcher's own calls.
// Removed the "My calls only" checkbox and the own/other call distinction logic
// since all calls returned are now always the logged-in dispatcher's own calls.
// ══════════════════════════════════════════════════════════════════════════════
const CallsTab = ({ showToast, userId }) => {
  ...
  // FIX: removed filterOwnOnly state — no longer needed since all calls are own calls
  ...
  const loadCalls = useCallback(async () => {
    setLoading(true);
    try {
      // FIX: use /dispatcher/calls (own calls only) — /dispatcher/calls/all does not exist
  ...
  // FIX: removed filterOwnOnly from filter logic — all calls are already own calls
  const filteredCalls = calls.filter(c => {
    if (filterStatus && c.call_status !== filterStatus) return false;
    if (searchCall.trim()) {
      const q = searchCall.trim().toLowerCase();
      const matchesReqId    = c.Request_id?.toLowerCase().includes(q);
      const matchesName     = c.victim_name?.toLowerCase().includes(q);
      const matchesPhone    = c.victim_phone?.includes(q);
      const matchesVictimId = c.Victim_id?.toLowerCase().includes(q);
      if (!matchesReqId && !matchesName && !matchesPhone && !matchesVictimId) return false;
    }
    return true;
  });
  ...
  // FIX: removed filterOwnOnly from clearFilters and hasFilters
  ...
  // ── List view ──
  // FIX: removed "My calls only" checkbox — all calls fetched are already own calls
  return (
    <div>
      <div style={D.filterBar}>
        <input
          ...
          placeholder="Search by Request ID, victim name, phone, or victim ID"
          style={{ ...D.input, flex:1 }}
        />
      ...
      {filteredCalls.length === 0
        ...
        // FIX: removed isOwn check — all calls are own calls; removed left-border distinction
        : filteredCalls.map(c => (
          <div key={c.Request_id} style={D.listRow}>
            ...
            <div style={D.listRowActions}>
              ...
              {/* FIX: all fetched calls belong to this dispatcher, so all are editable */}
      ...
      <div>
        ...
        <div style={D.detailCard}>
          ...
          {isCompleted ? (
            <div style={D.infoBox}>
              <p style={{ margin:0, fontSize:'13px', color:'#555' }}>
                🔒 This incident is <strong>Completed</strong> — no further edits are allowed.
              </p>
            </div>
          ...
          {isCompleted && detail.Assignment_id && (
            <>
              ...
              <div style={{ marginTop:'10px' }}>
                <div style={D.infoBox}>
                  <p style={{ margin:0, fontSize:'13px', color:'#555' }}>
                    🔒 Assignment is <strong>Completed</strong> — status changes and volunteer assignments are disabled.
                  </p>
                </div>
              </div>
            </>
          ...
          {!isCompleted && detail.Assignment_id && (
            <>
              ...
              {volunteers.length === 0 && assignedVols.length === 0 && (
                <p style={D.pendingNote}>No volunteers available at your centre.</p>
              )}
            </>
          )}
...
const CasesTab = ({ showToast }) => {
  ...
  const loadAll = useCallback(async () => {
    ...
    } catch (e) { setErr(e.response?.data?.error || 'Failed to load cases. Please try again.'); }
    finally { setLoading(false); }
  }, []);
        ...
        <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' }}>
          {STATUSES.map(s => (
            <button key={s} onClick={() => updateLawCase(managingLaw.Law_case_id, s)}
              ...
              style={{ ...D.btn, opacity: managingLaw.Status === s ? 0.35 : 1 }}>
        ...
        <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' }}>
          {STATUSES.map(s => (
            <button key={s} onClick={() => updateFollowup(managingFup.follow_up_id, s)}
              ...
              style={{ ...D.btn, opacity: managingFup.Status === s ? 0.35 : 1 }}>
...
const ResourcesTab = () => {
  ...
  const police    = (data?.police    || []).filter(p => (!filter || p.Status === filter) && matchLoc(p.Location));
  const ambulance = (data?.ambulance || []).filter(a => (!filter || a.Status === filter) && (matchLoc(a.Location) || matchLoc(a.Relevant_hospital)));
  ...
  return (
    <div>
      ...
      {subTab === 'police' && (police.length === 0 ? <Empty icon="👮" text="No police centres match filter." /> : police.map(p => (
        <div key={p.Centre_id} style={D.listRow}><div style={D.listRowLeft}><div style={{ display:'flex', gap:'8px', alignItems:'center' }}><span style={D.listId}>{p.Centre_id}</span>{statusBadge(p.Status)}</div><p style={D.listSub}>{p.Location} · {p.Centre_number}</p></div></div>
      )))}
      {subTab === 'ambulance' && (ambulance.length === 0 ? <Empty icon="🚑" text="No ambulances match filter." /> : ambulance.map(a => (
        <div key={a.Ambulance_id} style={D.listRow}><div style={D.listRowLeft}><div style={{ display:'flex', gap:'8px', alignItems:'center' }}><span style={D.listId}>{a.Ambulance_id}</span>{statusBadge(a.Status)}</div><p style={D.listSub}>{a.Relevant_hospital} · {a.Location} · {a.Contact_info}</p></div></div>
      )))}
    </div>
  );
};
...
const VolunteersTab = () => {
  ...
  return (
    <div>
      ...
      <Err msg={err} />
      {filtered.length === 0 ? <Empty icon="🤝" text={volunteers.length===0?'No volunteers at your centre.':'No volunteers match filters.'} />
        : filtered.map(v => (
