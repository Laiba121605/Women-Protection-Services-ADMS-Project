...
// Tabs: Overview | Recoveries | Dispatchers | Volunteers | Victims | Incidents | Users | System | Tables
...
const statusBadge = (s) => {
  const m = {
    ...
    Processed: { color:'#2980b9', bg:'#e8f4fd' },
    Resolved:  { color:'#1e8449', bg:'#eafaf1' },
    Arrived:   { color:'#8e44ad', bg:'#fdf2f8' },
    ...
    Emergency: { color:'#c0392b', bg:'#fff0f0' },
    Query:     { color:'#2980b9', bg:'#e8f4fd' },
  };
  if (!s) return <Badge label="—" color="#bbb" bg="#f5f5f5" />;
...
const Modal = ({ title, onClose, children }) => (
  <div style={D.modalOverlay}>
    <div style={D.modalBox}>
      <div style={D.modalHeader}>
        <span style={D.modalTitle}>{title}</span>
        <button onClick={onClose} style={D.modalClose}>✕</button>
      </div>
      <div style={D.modalBody}>{children}</div>
    </div>
  </div>
);
...
const Confirm = ({ message, onConfirm, onCancel, loading }) => (
  <div style={D.modalOverlay}>
    <div style={{ ...D.modalBox, maxWidth:'420px' }}>
      <div style={D.modalBody}>
        <p style={{ fontSize:'14px', color:'#333', margin:'0 0 20px', lineHeight:'1.6' }}>{message}</p>
        <div style={{ display:'flex', gap:'10px', justifyContent:'flex-end' }}>
          <button onClick={onCancel} style={D.bannerCancel} disabled={loading}>Cancel</button>
          <button onClick={onConfirm} disabled={loading}
            style={{ ...D.btnPrimary, backgroundColor:'#c0392b', fontSize:'13px' }}>
...
// ══════════════════════════════════════════════════════════════════════════════
// RECOVERIES TAB — CHANGED: removed mineOnly, simplified filter bar
// ══════════════════════════════════════════════════════════════════════════════
const RecoveriesTab = ({ showToast }) => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const url = filter ? `/admin/recoveries?status=${filter}` : '/admin/recoveries';
    ...
    <div>
      ...
      {newPassword && (
        <div style={{ backgroundColor:'#eafaf1', border:'1.5px solid #a9dfbf', borderRadius:'10px', padding:'14px 18px', marginBottom:'16px' }}>
          <p style={{ margin:'0 0 6px', fontSize:'13px', fontWeight:'700', color:'#1e8449' }}>✅ Password reset for {newPassword.email}</p>
        ...
        : filtered.map(r => (
          <div key={r.Recovery_id} style={D.listRow}>
            <div style={D.listRowLeft}>
              ...
              {r.Response_time && <p style={D.listSub}>Responded: {fmt(r.Response_time)} by {r.admin_name || 'Admin'}</p>}
              {r.Note && <p style={{ ...D.listSub, fontStyle:'italic', color:'#888' }}>Note: {r.Note}</p>}
            </div>
...
const DispatchersTab = ({ showToast }) => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRows(r.data); setDispatchers(r.data);
    }
  ...
  const reassignAll = async (fromId) => {
    ...
    try {
      ...
      const myActive = incRes.data.filter(i => i.Dispatcher_id === fromId && i.incident_status !== 'Completed');
      if (myActive.length === 0) { showToast('No active incidents to reassign'); setReassigning(null); return; }
      await Promise.all(myActive.map(i => API.put(`/admin/incident/${i.Incident_id}/reassign`, { dispatcher_id: toId })));
  ...
  return (
    <div>
      ...
      <Err msg={err} />
      {filtered.length === 0 ? <Empty icon="📞" text="No dispatchers found." /> : filtered.map(d => (
        <div key={d.User_id} style={D.listRow}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'8px', alignItems:'flex-end', flexShrink:0 }}>
            <button onClick={() => setAvail(d.User_id, d.Availability==='Yes'?'No':'Yes')} disabled={updating===d.User_id}
            ...
            {blockErr[d.User_id] && (
              <div style={{ maxWidth:'320px', textAlign:'right' }}>
                <p style={{ margin:'0 0 6px', fontSize:'12px', color:'#c0392b', fontWeight:'600' }}>⚠️ {blockErr[d.User_id]}</p>
...
const VolunteersTab = () => {
  ...
  const centres = [...new Set(rows.map(r => r.centre_location).filter(Boolean))].sort();

  const filtered = rows.filter(r => {
    if (filterAvail   && r.Availability !== filterAvail) return false;
    if (filterStatus === 'none' && r.Status != null) return false;
    if (filterStatus && filterStatus !== 'none' && r.Status !== filterStatus) return false;
  ...
  return (
    <div>
      <div style={D.filterBar}>
        ...
        <select value={filterStatus} onChange={e=>setFilterStatus(e.target.value)} style={{ ...D.select, width:'150px', flex:'none' }}>
          <option value="">All statuses</option>
        ...
        {(search||filterAvail||filterStatus||filterCentre) && (
          <button onClick={()=>{setSearch('');setFilterAvail('');setFilterStatus('');setFilterCentre('');}} style={D.clearBtn}>✕ Clear</button>
        )}
      </div>
      <div style={D.listHeader}>
      ...
      <Err msg={err} />
      {filtered.length === 0 ? <Empty icon="🤝" text="No volunteers found." /> : filtered.map(v => (
        <div key={v.User_id} style={D.listRow}>
          <div style={D.listRowLeft}>
            <div style={{ display:'flex', gap:'8px', alignItems:'center', flexWrap:'wrap' }}>
              <span style={D.listId}>{v.User_id}</span>
              {statusBadge(v.Availability)}
              {v.Status != null ? statusBadge(v.Status) : <Badge label="Not assigned" color="#aaa" bg="#f5f5f5" />}
            </div>
            <p style={{ margin:'4px 0 2px', fontSize:'13px', fontWeight:'600', color:'#333' }}>{v.Name}</p>
            <p style={D.listSub}>{v.Phone_no} · {v.Email} · {v.centre_location} · {v.total_assignments} assignment{v.total_assignments !== 1 ? 's' : ''}</p>
          </div>
        </div>
      ))}
    </div>
...
const VictimsTab = () => {
  ...
  return (
    <div>
      <div style={D.filterBar}>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search by name, ID, phone, email or CNIC" style={{ ...D.input, flex:1 }} />
        <select value={filterActive} onChange={e=>setFilterActive(e.target.value)} style={{ ...D.select, width:'190px', flex:'none' }}>
      ...
      <Err msg={err} />
      {filtered.length === 0 ? <Empty icon="🛡️" text="No victims found." /> : filtered.map(v => (
        <div key={v.User_id} style={D.listRow}>
          <div style={D.listRowLeft}>
            <div style={{ display:'flex', gap:'8px', alignItems:'center', flexWrap:'wrap' }}>
              <span style={D.listId}>{v.User_id}</span>
            ...
            <p style={{ margin:'4px 0 2px', fontSize:'13px', fontWeight:'600', color:'#333' }}>{v.Name}</p>
            <p style={D.listSub}>{v.Phone_no} · {v.Email} · CNIC: {v.CNIC} · {v.total_requests} request{v.total_requests!==1?'s':''} · EC: {v.Emergency_contact}</p>
          </div>
        </div>
      ))}
    </div>
...
const IncidentsTab = ({ showToast }) => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRows(iRes.data); setDispatchers(dRes.data);
  ...
  const filtered = rows.filter(r => {
    ...
    if (filterType   && r.Emergency_type  !== filterType)   return false;
    if (search.trim() &&
  ...
  return (
    <div>
      <div style={D.filterBar}>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search by ID, victim, dispatcher or location" style={{ ...D.input, flex:1 }} />
        <select value={filterSev} onChange={e=>setFilterSev(e.target.value)} style={{ ...D.select, width:'130px', flex:'none' }}>
          <option value="">All severities</option>
          {['High','Medium','Low'].map(s=><option key={s}>{s}</option>)}
        </select>
        <select value={filterStatus} onChange={e=>setFilterStatus(e.target.value)} style={{ ...D.select, width:'130px', flex:'none' }}>
          <option value="">All statuses</option>
          {['Ongoing','Pending','Completed'].map(s=><option key={s}>{s}</option>)}
        </select>
        <select value={filterType} onChange={e=>setFilterType(e.target.value)} style={{ ...D.select, width:'180px', flex:'none' }}>
          <option value="">All types</option>
          {EMERGENCY_TYPES.map(t=><option key={t}>{t}</option>)}
        </select>
        {(search||filterSev||filterStatus||filterType) && (
          <button onClick={()=>{setSearch('');setFilterSev('');setFilterStatus('');setFilterType('');}} style={D.clearBtn}>✕ Clear</button>
        )}
      </div>
      <div style={D.listHeader}>
      ...
      <Err msg={err} />
      {filtered.length === 0 ? <Empty icon="🚨" text="No incidents found." /> : filtered.map(i => (
        <div key={i.Incident_id} style={D.listRow}>
          <div style={D.listRowLeft}>
            <div style={{ display:'flex', gap:'8px', alignItems:'center', flexWrap:'wrap' }}>
              <span style={D.listId}>{i.Incident_id}</span>
              {statusBadge(i.Severity)}
              {statusBadge(i.incident_status)}
            </div>
            <p style={{ margin:'4px 0 2px', fontSize:'13px', color:'#333' }}>
              <strong>{i.Emergency_type}</strong> · Victim: <strong>{i.victim_name}</strong> ({i.victim_phone}) · Dispatcher: <strong>{i.dispatcher_name}</strong>
            </p>
            <p style={D.listSub}>{i.incident_location || i.centre_location} · {fmt(i.incident_time)}</p>
          </div>
          {i.incident_status !== 'Completed' && (
            <div style={{ display:'flex', gap:'6px', alignItems:'center', flexShrink:0 }}>
              <select value={selDispatcher[i.Incident_id]||''} onChange={e=>setSelDispatcher(prev=>({...prev,[i.Incident_id]:e.target.value}))}
                style={{ ...D.select, width:'160px', fontSize:'12px', padding:'5px 8px' }}>
...
// USERS TAB
// ══════════════════════════════════════════════════════════════════════════════
const ROLES = ['Admin','Victim','Volunteer','Dispatcher'];

const CreateUserModal = ({ onClose, onCreated, showToast }) => {
  ...
  useEffect(() => {
    (async () => {
      try { const r = await API.get('/admin/centre'); setCentres(r.data?.data || []); }
      catch { /* non-critical */ }
    })();
  }, []);

  const set = (k, v) => { setFields(p => ({ ...p, [k]: v })); setErr(''); };

  const submit = async () => {
    setSaving(true); setErr('');
    try {
      const res = await API.post('/admin/user', fields);
      showToast(`User created: ${res.data.user_id}`);
      onCreated(); onClose();
    } catch (e) { setErr(e.response?.data?.error || 'Failed to create user'); }
    ...
    <Modal title="➕ Create New User" onClose={onClose}>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px' }}>
        {[
          ...
          { key:'phone_no',      label:'Phone (03…) *',           type:'text' },
          { key:'cnic',          label:'CNIC (xxxxx-xxxxxxx-x) *', type:'text' },
          { key:'address',       label:'Address *',                type:'text',     span:2 },
          { key:'date_of_birth', label:'Date of Birth *',          type:'date' },
          { key:'password',      label:'Password *',               type:'password' },
        ].map(f => (
          <div key={f.key} style={{ gridColumn: f.span ? `span ${f.span}` : undefined }}>
            <label style={D.formLabel}>{f.label}</label>
            <input type={f.type} value={fields[f.key]||''} onChange={e=>set(f.key,e.target.value)} style={D.input} />
          </div>
        ))}
        <div>
          <label style={D.formLabel}>Role *</label>
          <select value={fields.role||''} onChange={e=>set('role',e.target.value)} style={D.select}>
            <option value="">— Select role —</option>
            {ROLES.map(r=><option key={r}>{r}</option>)}
          </select>
        </div>
        {(fields.role==='Volunteer'||fields.role==='Dispatcher') && (
          <div>
            <label style={D.formLabel}>Centre *</label>
            <select value={fields.centre_id||''} onChange={e=>set('centre_id',e.target.value)} style={D.select}>
              <option value="">— Select centre —</option>
              {centres.map(c=><option key={c.Centre_id} value={c.Centre_id}>{c.Location}</option>)}
            </select>
          </div>
        )}
        {(fields.role==='Victim'||fields.role==='Volunteer') && (
          <div style={{ gridColumn:'span 2' }}>
            <label style={D.formLabel}>Emergency Contact * (different from personal phone)</label>
            <input type="text" value={fields.emergency_contact||''} onChange={e=>set('emergency_contact',e.target.value)}
              placeholder="03…" style={D.input} />
          </div>
        )}
        {err && <div style={{ gridColumn:'span 2' }}><Err msg={err} /></div>}
      </div>
      <div style={{ display:'flex', gap:'10px', justifyContent:'flex-end', marginTop:'20px' }}>
        <button onClick={onClose} style={D.bannerCancel} disabled={saving}>Cancel</button>
        <button onClick={submit} disabled={saving} style={D.btnPrimary}>{saving ? 'Creating…' : 'Create User'}</button>
      </div>
    </Modal>
  );
};
...
const EditUserModal = ({ user, onClose, onSaved, showToast }) => {
  const [fields, setFields] = useState({
    name: user.Name, email: user.Email, phone_no: user.Phone_no || user.Phone,
    address: user.Address, cnic: user.CNIC,
    date_of_birth: user.Date_of_Birth?.split('T')[0] || '',
    password: '',
    emergency_contact: user.Victim_Emergency_contact || user.Volunteer_Emergency_contact || '',
    centre_id: user.Volunteer_Centre_id || '',
  });
  const [saving, setSaving]   = useState(false);
  const [err, setErr]         = useState('');
  const [centres, setCentres] = useState([]);
  ...
  const submit = async () => {
    ...
    try {
      await API.put(`/admin/user/${user.User_id}`, payload);
      showToast('User updated');
      onSaved(); onClose();
    } catch (e) { setErr(e.response?.data?.error || 'Failed to update user'); }
    finally { setSaving(false); }
  };
  ...
  return (
    <Modal title={`✏️ Edit — ${user.Name}`} onClose={onClose}>
      <p style={{ margin:'0 0 16px', fontSize:'12px', color:'#aaa' }}>
        Leave Password blank to keep existing. To change role, delete and recreate the user.
      </p>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px' }}>
        {[
          { key:'name',          label:'Full Name',          type:'text',     span:2 },
          ...
          <div key={f.key} style={{ gridColumn: f.span ? `span ${f.span}` : undefined }}>
            ...
            <input type={f.type} value={fields[f.key]||''} onChange={e=>set(f.key,e.target.value)}
              style={D.input} placeholder={f.key==='password'?'Leave blank to keep':''} />
          </div>
        ))}
        {(user.Role==='Victim'||user.Role==='Volunteer') && (
          <div style={{ gridColumn:'span 2' }}>
            <label style={D.formLabel}>Emergency Contact</label>
            <input type="text" value={fields.emergency_contact||''} onChange={e=>set('emergency_contact',e.target.value)} style={D.input} />
          </div>
        )}
        {(user.Role==='Volunteer'||user.Role==='Dispatcher') && (
          <div style={{ gridColumn:'span 2' }}>
            <label style={D.formLabel}>Centre</label>
            <select value={fields.centre_id||''} onChange={e=>set('centre_id',e.target.value)} style={D.select}>
      ...
      <div style={{ display:'flex', gap:'10px', justifyContent:'flex-end', marginTop:'20px' }}>
        <button onClick={onClose} style={D.bannerCancel} disabled={saving}>Cancel</button>
        <button onClick={submit} disabled={saving} style={D.btnPrimary}>{saving ? 'Saving…' : 'Save Changes'}</button>
      </div>
    </Modal>
  );
};
...
const UsersTab = ({ showToast }) => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRows(r.data.users || r.data);
    } catch (e) { setErr(e.response?.data?.error || 'Failed to load users'); }
    finally { setLoading(false); }
  }, []);

  const doSearch = useCallback(async () => {
    if (!search.trim() && !filterRole) return load();
    setLoading(true);
    try {
      const params = new URLSearchParams();
      if (search.trim()) params.set('name', search.trim());
      if (filterRole)    params.set('role', filterRole);
      const r = await API.get(`/admin/users/search?${params}`);
      setRows(r.data);
  ...
  const deleteUser = async () => {
    setDeleting(true);
    try {
      await API.delete(`/admin/user/${confirmDel.User_id}`);
      showToast(`User ${confirmDel.Name} deleted`);
      setConfirmDel(null);
      load();
    } catch (e) {
  ...
  const roleColor = {
    Admin:      { color:'#7b2d8b', bg:'#f5e8fa' },
    Victim:     { color:'#c0392b', bg:'#fff0f0' },
    Volunteer:  { color:'#1e8449', bg:'#eafaf1' },
    Dispatcher: { color:'#2980b9', bg:'#e8f4fd' },
  };
  ...
  return (
    <div>
      {showCreate && <CreateUserModal onClose={()=>setShowCreate(false)} onCreated={load} showToast={showToast} />}
      {editUser   && <EditUserModal   user={editUser} onClose={()=>setEditUser(null)} onSaved={load} showToast={showToast} />}
      {confirmDel && (
        <Confirm
          message={`Delete "${confirmDel.Name}" (${confirmDel.Role})? This cannot be undone and will fail if they have open cases.`}
          onConfirm={deleteUser} onCancel={()=>setConfirmDel(null)} loading={deleting}
        />
      )}
      <div style={D.filterBar}>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search by name, email, phone…" style={{ ...D.input, flex:1 }} />
        <select value={filterRole} onChange={e=>setFilterRole(e.target.value)} style={{ ...D.select, width:'160px', flex:'none' }}>
          <option value="">All roles</option>
          {ROLES.map(r=><option key={r}>{r}</option>)}
        </select>
        {(search||filterRole) && <button onClick={()=>{setSearch('');setFilterRole('');}} style={D.clearBtn}>✕ Clear</button>}
        <button onClick={()=>setShowCreate(true)} style={D.addBtn}>➕ Add User</button>
      </div>
      <div style={D.listHeader}>
        <p style={D.listCount}>{rows.length} user{rows.length!==1?'s':''}</p>
      </div>
      <Err msg={err} />
      {rows.length === 0 ? <Empty icon="👤" text="No users found." /> : rows.map(u => {
        const rc = roleColor[u.Role] || { color:'#555', bg:'#f5f5f5' };
        return (
          <div key={u.User_id} style={D.listRow}>
            <div style={D.listRowLeft}>
              <div style={{ display:'flex', gap:'8px', alignItems:'center', flexWrap:'wrap' }}>
                <span style={D.listId}>{u.User_id}</span>
                <Badge label={u.Role} color={rc.color} bg={rc.bg} />
                {u.Availability && statusBadge(u.Availability)}
                {u.Status && statusBadge(u.Status)}
              </div>
              <p style={{ margin:'4px 0 2px', fontSize:'13px', fontWeight:'600', color:'#333' }}>{u.Name}</p>
              <p style={D.listSub}>
                {u.Email} · {u.Phone || u.Phone_no}
                {u.Centre ? ` · ${u.Centre}` : ''}
                {u.CNIC ? ` · ${u.CNIC}` : ''}
              </p>
            </div>
            <div style={{ display:'flex', gap:'6px', flexShrink:0 }}>
              <button onClick={()=>setEditUser(u)} style={{ ...D.smallBtn, color:'#2980b9', border:'1.5px solid #bee3f8' }}>✏️ Edit</button>
              <button onClick={()=>setConfirmDel(u)} style={{ ...D.smallBtn, color:'#c0392b', border:'1.5px solid #fecaca' }}>🗑️ Delete</button>
            </div>
          </div>
        );
      })}
  ...
  return (
    <div style={{ maxWidth:'700px' }}>
      <div style={{ ...D.statCard, marginBottom:'24px', border: active ? '2px solid #fecaca' : '1px solid #eef0f3', backgroundColor: active ? '#fff8f8' : '#fff' }}>
        ...
        {active && <p style={{ margin:'10px 0 0', fontSize:'12px', color:'#888' }}>All dispatcher write operations are blocked system-wide.</p>}
      </div>
      <Err msg={err} />
      {!active ? (
        <div style={D.formCard}>
          ...
          <p style={{ fontSize:'13px', color:'#555', margin:'0 0 14px' }}>
            Activating the lock blocks <strong>all dispatcher write operations</strong> across the entire system.
          </p>
          <div style={{ backgroundColor:'#f0faf4', border:'1px solid #a9dfbf', borderRadius:'8px', padding:'10px 14px', marginBottom:'16px' }}>
            ...
            <ul style={{ margin:0, paddingLeft:'18px', fontSize:'12px', color:'#555', lineHeight:'1.7' }}>
              <li>Update any record in the <strong>Tables</strong> tab</li>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'16px' }}>
            ...
            <input type="number" value={duration} onChange={e=>setDuration(Number(e.target.value))} min={1} max={60} style={{ ...D.input, width:'80px' }} />
          </div>
          <button onClick={activate} disabled={activating} style={{ ...D.btnPrimary, backgroundColor:'#c0392b' }}>
            {activating ? 'Activating…' : '🔒 Activate Database-Wide Lock'}
          </button>
        </div>
      ) : (
        <div style={D.formCard}>
          <SectionTitle>Release Priority Lock</SectionTitle>
          <p style={{ fontSize:'13px', color:'#888', margin:'0 0 16px' }}>Releasing the lock immediately allows dispatchers to resume write operations.</p>
          <p style={{ fontSize:'13px', color:'#555', margin:'0 0 16px' }}>💡 Use the <strong>Tables</strong> tab to view and edit any table while the lock is active.</p>
...
// ══════════════════════════════════════════════════════════════════════════════
// TABLES TAB — CHANGED: deleteRow uses setSaveErr (inline) not setErr (global banner)
// FK errors translated to human-readable messages. Del button next to Edit.
// ══════════════════════════════════════════════════════════════════════════════
const DB_VIEWS   = new Set(['volunteer_assigned_view','victim_personal_view']);
const UPDATABLE  = new Set([
  'request_log','law_case','follow_up_support','centre','incident','assignment',
  'ambulance_service','police_services','dispatcher','victim','user',
  'volunteer','password_recovery','volunteer_assignment',
]);
const ALL_BASE_TABLES = new Set([
...
const TablesTab = ({ showToast }) => {
  ...
  // Delete state — error goes to saveErr, NOT to err (global banner)
  const [confirmDelRow, setConfirmDelRow] = useState(null); // { idField, idValue }
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const baseTables = tablesRes.data.filter(t =>
          !DB_VIEWS.has(t.TABLE_NAME.toLowerCase()) &&
          ALL_BASE_TABLES.has(t.TABLE_NAME.toLowerCase())
        );
  ...
  useEffect(() => {
    const id = setInterval(async () => {
      try { const r = await API.get('/admin/priority-lock-status'); setLockActive(r.data.active === true); }
      catch { /* ignore */ }
    }, 15000);
  ...
  const loadTable = async (name) => {
    setSelectedTable(name); setLoadingTable(true); setTableData(null);
    setSearch(''); setEditingRow(null); setSaveErr(''); setConfirmDelRow(null);
    try {
  ...
  const startEdit = (row) => { setEditingRow(row); setEditFields({ ...row }); setSaveErr(''); };

  const saveEdit = async (idField, idValue) => {
    setSaving(true); setSaveErr('');
    const changed = {};
    Object.keys(editFields).forEach(k => { if (editFields[k] !== editingRow[k]) changed[k] = editFields[k]; });
    if (Object.keys(changed).length === 0) { setSaveErr('No changes made.'); setSaving(false); return; }
    try {
      await API.put(`/admin/${selectedTable}/${idValue}`, changed);
      showToast('Record updated'); setEditingRow(null);
      await loadTable(selectedTable);
    } catch (e) { setSaveErr(e.response?.data?.error || 'Save failed'); }
    finally { setSaving(false); }
  };

  // Translate raw MySQL FK errors into readable messages
  const humanizeDbError = (msg) => {
    if (!msg) return 'Delete failed.';
    if (msg.includes('foreign key') || msg.includes('FOREIGN KEY') || msg.includes('constraint fails')) {
      const refMatch = msg.match(/REFERENCES `([^`]+)`/i);
      const refTable = refMatch?.[1];
      if (refTable && refTable !== selectedTable) {
        return `Cannot delete — this record is still referenced by "${refTable}". Remove or reassign all linked ${refTable} records first.`;
      }
      return 'Cannot delete — this record is still referenced by other records. Remove all linked records first.';
    }
    if (msg.includes('Duplicate entry')) return 'Cannot save — a duplicate value already exists.';
    return msg;
  };

  // DELETE row — error shown inline via setSaveErr, NOT the global err banner
  const deleteRow = async () => {
    if (!confirmDelRow) return;
    setDeletingRow(true); setSaveErr('');
    try {
      await API.delete(`/admin/${selectedTable}/${confirmDelRow.idValue}`);
      showToast('Record deleted'); setConfirmDelRow(null);
      await loadTable(selectedTable);
    } catch (e) {
      const raw = e.response?.data?.error || e.response?.data?.message || 'Delete failed';
      setSaveErr(humanizeDbError(raw));
  ...
  return (
    <div>
      ...
      {confirmDelRow && (
        <div style={{ position:'fixed', inset:0, backgroundColor:'rgba(0,0,0,0.45)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:10000, padding:'20px' }}>
          <div style={{ backgroundColor:'#fff', borderRadius:'14px', width:'100%', maxWidth:'420px', padding:'28px', boxShadow:'0 20px 60px rgba(0,0,0,0.2)' }}>
            <p style={{ margin:'0 0 8px', fontSize:'15px', fontWeight:'700', color:'#1a1a2e' }}>Delete record?</p>
            <p style={{ margin:'0 0 20px', fontSize:'13px', color:'#555', lineHeight:'1.6' }}>
              Delete <code style={{ backgroundColor:'#f5f5f5', padding:'1px 6px', borderRadius:'4px', fontFamily:'monospace' }}>{confirmDelRow.idValue}</code> from <strong>{selectedTable}</strong>?
              This cannot be undone and will fail if the record has dependencies.
            </p>
            <div style={{ display:'flex', gap:'10px', justifyContent:'flex-end' }}>
              <button onClick={() => setConfirmDelRow(null)} disabled={deletingRow} style={D.bannerCancel}>Cancel</button>
              <button onClick={deleteRow} disabled={deletingRow} style={{ ...D.btnPrimary, backgroundColor:'#c0392b' }}>
      ...
      {!lockChecking && (
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'10px 16px', borderRadius:'10px', marginBottom:'16px', backgroundColor: lockActive ? '#eafaf1' : '#fffbf0', border:`1px solid ${lockActive ? '#a9dfbf' : '#f9e4b7'}` }}>
          <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
            <span style={{ fontSize:'16px' }}>{lockActive ? '🔒' : '🔓'}</span>
            <span style={{ fontSize:'13px', fontWeight:'600', color: lockActive ? '#1e8449' : '#b7770d' }}>
              {lockActive ? 'Priority lock is active — table editing & deleting enabled' : 'Priority lock is OFF — enable it from the System tab to make changes.'}
            </span>
          </div>
          {!lockActive && <span style={{ fontSize:'12px', color:'#aaa' }}>Go to System tab → Enable Lock</span>}
        </div>
      )}
      ...
      {!selectedTable ? (
        <>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'16px', flexWrap:'wrap', gap:'8px' }}>
            <p style={{ ...D.listCount, margin:0 }}>{tables.length} base tables</p>
            <span style={{ fontSize:'12px', color:'#bbb', fontStyle:'italic' }}>Database views excluded — only manageable base tables shown</span>
          </div>
            ...
            {tables.map(t => (
              <div key={t.TABLE_NAME} onClick={() => loadTable(t.TABLE_NAME.toLowerCase())}
                ...
                <p style={{ margin:0, fontSize:'12px', color:'#aaa' }}>
                  ...
                  {UPDATABLE.has(t.TABLE_NAME.toLowerCase()) ? (lockActive ? ' · ✏️🗑️ editable' : ' · 🔒 locked') : ' · 👁️ read only'}
                </p>
              </div>
            ))}
          </div>
        ...
        <>
          <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'16px', flexWrap:'wrap' }}>
            ...
            {canEdit ? <Badge label="✏️🗑️ Edit & Delete" color="#1e8449" bg="#eafaf1" /> : <Badge label="👁️ Read only" color="#888" bg="#f5f5f5" />}
          </div>

          {loadingTable ? <Spinner /> : tableData && (
            <>
              <div style={D.filterBar}>
                <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search all columns…" style={{ ...D.input, flex:1 }} />
                {search && <button onClick={()=>setSearch('')} style={D.clearBtn}>✕ Clear</button>}
              </div>
              <p style={{ ...D.listCount, marginBottom:'12px' }}>{filteredData.length} of {tableData.row_count} rows</p>

              {/* Inline error for delete failures — stays inside the tab, NOT global banner */}
              {saveErr && !editingRow && (
                <div style={{ display:'flex', alignItems:'center', gap:'8px', backgroundColor:'#fff5f5', color:'#c0392b', padding:'10px 14px', borderRadius:'8px', fontSize:'13px', border:'1px solid #fecaca', marginBottom:'12px' }}>
                  ⚠️ {saveErr}
                  <button onClick={()=>setSaveErr('')} style={{ marginLeft:'auto', background:'none', border:'none', color:'#c0392b', cursor:'pointer', fontSize:'16px', lineHeight:1 }}>✕</button>
                </div>
              )}
                              ...
                              {tableData.columns.map(col => (
                                <td key={col} style={{ padding:'8px 12px', color:'#333', whiteSpace:'nowrap', maxWidth:'200px', overflow:'hidden', textOverflow:'ellipsis' }}>
                                  {isEditing && col !== idField
                                    ? <input value={editFields[col] ?? ''} onChange={e=>setEditFields(prev=>({...prev,[col]:e.target.value}))}
                                ...
                                <td style={{ padding:'8px 12px', whiteSpace:'nowrap' }}>
                                  {isEditing ? (
                                    ...
                                    <div style={{ display:'flex', gap:'4px' }}>
                                      ...
                                      <button onClick={()=>{ setConfirmDelRow({ idField, idValue }); setSaveErr(''); }}
                                        style={{ padding:'3px 10px', borderRadius:'4px', border:'1.5px solid #fecaca', backgroundColor:'#fff5f5', color:'#c0392b', fontSize:'11px', cursor:'pointer' }}>
                                        Del
                                      </button>
                                    </div>
                                  )}
...
const D = {
  ...
  statCard:     { backgroundColor:'#fff', borderRadius:'12px', padding:'20px', border:'1px solid #eef0f3', boxShadow:'0 1px 6px rgba(0,0,0,0.04)ило' },
  ...
  addBtn:       { padding:'8px 16px', borderRadius:'8px', border:'none', backgroundColor:'#7b2d8b', color:'#fff', fontSize:'12px', fontWeight:'700', cursor:'pointer', whiteSpace:'nowrap', marginLeft:'auto' },
  ...
  formLabel:    { display:'block', fontSize:'12px', fontWeight:'700', color:'#555', marginBottom:'5px' },
  ...
  modalOverlay: { position:'fixed', inset:0, backgroundColor:'rgba(0,0,0,0.45)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:10000, padding:'20px' },
  modalBox:     { backgroundColor:'#fff', borderRadius:'14px', width:'100%', maxWidth:'640px', maxHeight:'90vh', display:'flex', flexDirection:'column', boxShadow:'0 20px 60px rgba(0,0,0,0.2)' },
  modalHeader:  { display:'flex', alignItems:'center', justifyContent:'space-between', padding:'18px 24px', borderBottom:'1px solid #eef0f3' },
  modalTitle:   { fontSize:'15px', fontWeight:'700', color:'#1a1a2e' },
  modalClose:   { background:'none', border:'none', fontSize:'16px', color:'#aaa', cursor:'pointer', padding:'4px 8px', borderRadius:'6px' },
