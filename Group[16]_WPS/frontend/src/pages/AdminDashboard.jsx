...
const statusBadge = (s) => {
  const m = {
    ...
    Processed: { color:'#2980b9', bg:'#e8f4fd' }, // FIX: added Processed (backend value for resolved recoveries)
...
const RecoveriesTab = ({ showToast }) => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      // FIX: pass filter value directly — backend uses 'Pending' and 'Processed'
  ...
  return (
    <div>
      <div style={D.filterBar}>
        ...
        {/* FIX: filter values now match backend Status column — 'Pending' and 'Processed' */}
        ...
        : filtered.map(r => (
          <div key={r.Recovery_id} style={D.listRow}>
            <div style={D.listRowLeft}>
              ...
              {/* FIX: backend column is Response_time, not Resolution_time */}
              {r.Response_time && (
                <p style={D.listSub}>Responded: {fmt(r.Response_time)} by {r.admin_name || 'Admin'}</p>
              )}
            </div>
...
// FIX: Removed the fake table/row scope selector UI. The backend only supports
// a single database-wide app-level lock via POST /admin/priority-lock.
// There is no backend support for per-table or per-row priority locking through
// this endpoint, so those options were removed to prevent misleading the user.
// ══════════════════════════════════════════════════════════════════════════════
      ...
      <div style={{ ...D.statCard, marginBottom:'24px', border: active ? '2px solid #fecaca' : '1px solid #eef0f3', backgroundColor: active ? '#fff8f8' : '#fff' }}>
        ...
        {active && (
          <p style={{ margin:'10px 0 0', fontSize:'12px', color:'#888' }}>
            All dispatcher write operations are blocked system-wide. You (the admin) can still write freely.
          </p>
        )}
      </div>
      ...
      {!active ? (
        <div style={D.formCard}>
          ...
          <p style={{ fontSize:'13px', color:'#555', margin:'0 0 14px' }}>
            ...
            Use this during maintenance windows or when you need exclusive write access.
          </p>
