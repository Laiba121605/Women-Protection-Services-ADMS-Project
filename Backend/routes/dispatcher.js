...
router.post('/victim', async (req, res) => {
    ...
    try {
        ...
        if (!phone_no.startsWith('03')) throw new Error('Phone number must start with 03.');

        // Check email format if provided
        if (email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
            throw new Error('Invalid email format.');

        // Check emergency contact format if provided
        if (emergency_contact) {
            ...
            if (!emergency_contact.startsWith('03'))
                throw new Error('Emergency contact must start with 03.');
            if (emergency_contact === phone_no)
                throw new Error('Emergency contact cannot be the same as personal phone number.');
        }
        ...
        // Also check if phone is being used as someone's emergency contact
        // to prevent accidentally creating a duplicate identity
        if (!existingPhone.length) {
            const [usedAsEC] = await connection.query(
                'SELECT User_id FROM Victim WHERE Emergency_contact = ?', [phone_no]
            );
            if (usedAsEC.length) {
                logger.info(`PHONE_IS_EMERGENCY_CONTACT - Phone: ${phone_no} is EC for User: ${usedAsEC[0].User_id}`);
                // Not a hard block — emergency contacts can also be registered users
                // but log it so dispatcher is aware
            }
        }
...
router.post('/followup', async (req, res) => {
    ...
    try {
        ...
        // Auto-assign from a previously used Referred_centre in Follow_up_support
        // if none provided. Falls back to a generic default if table is empty.
        if (!referred_centre || !referred_centre.trim()) {
            const [[prevCentre]] = await connection.query(
                `SELECT Referred_centre FROM Follow_up_support
                 ORDER BY follow_up_id DESC LIMIT 1`
            );
            referred_centre = prevCentre
                ? prevCentre.Referred_centre
...
// ======================================================
// 🔹 GET EXISTING LAW FIRMS from Law_case table
// Used to populate the law firm dropdown with previously used names
// ======================================================
router.get('/lawfirms', async (req, res) => {
    try {
        const [rows] = await pool.query(`
            SELECT DISTINCT Lawfirm_name
            FROM Law_case lc
            JOIN Incident i    ON lc.Incident_id = i.Incident_id
            JOIN Request_log r ON i.Request_id   = r.Request_id
            WHERE r.Dispatcher_id = ?
...
// ======================================================
// 🔹 GET EXISTING REFERRED CENTRES from Follow_up_support table
// Used to populate the referred centre dropdown with previously used names.
// Follow_up_support.Referred_centre is a free-text varchar — NOT a foreign key
// to the Centre table. The Centre table is for dispatcher/volunteer/admin centres.
// ======================================================
router.get('/referred-centres', async (req, res) => {
    try {
        const [rows] = await pool.query(`
            SELECT DISTINCT fs.Referred_centre
            FROM Follow_up_support fs
            JOIN Assignment a  ON fs.Assignment_id = a.Assignment_id
            JOIN Incident i    ON a.Incident_id    = i.Incident_id
            JOIN Request_log r ON i.Request_id     = r.Request_id
...
// ======================================================
// 🔹 GET ALL VOLUNTEERS (not just available — full list for dispatcher view)
// ======================================================
router.get('/volunteers', async (req, res) => {
    try {
        ...
        const [rows] = await pool.query(`
            ...
            WHERE v.Centre_id = ?
            GROUP BY v.User_id, u.Name, u.Phone_no, u.Email,
                     v.Status, v.Availability, v.Emergency_contact,
                     v.Centre_id, c.Location
...
// ======================================================
// 🔹 GET ALL RESOURCES (police + ambulance with all statuses)
// ======================================================
router.get('/resources', async (req, res) => {
    try {
        const [police] = await pool.query(`
            SELECT Centre_id, Location, Centre_number, Status
            FROM Police_services
            ORDER BY Status ASC
        `);
        const [ambulance] = await pool.query(`
            SELECT Ambulance_id, Relevant_hospital, Location, Contact_info, Status
            FROM Ambulance_service
            ORDER BY Status ASC
        `);
...
// ======================================================
// 🔹 GET ALL ACTIVE BACKUP REQUESTS across all incidents
// Returns incidents where the Note contains BACKUP REQUESTED
// so the dispatcher can see all pending backup situations at once
// ======================================================
router.get('/backups', async (req, res) => {
    try {
        ...
        const result = rows.map(row => {
            ...
            // Split on both newline (dispatcher format) and pipe separator (volunteer format)
            const lines = notes.split(/\n| \| /).filter(l => l.includes('BACKUP REQUESTED'));
            const backupEntries = lines.map(l => {
                // Format A (volunteer.js):  "BACKUP REQUESTED by Name: reason"
                // Format B (dispatcher.js): "[timestamp] BACKUP REQUESTED by Volunteer Name: reason ..."
                const withTs  = l.match(/\[(.*?)\] BACKUP REQUESTED by (?:Volunteer )?(.+?): (.*)/);
                const withoutTs = l.match(/BACKUP REQUESTED by (.+?)(?:: (.*))?$/);
                if (withTs)    return { timestamp: withTs[1],  volunteer: withTs[2],    details: withTs[3] || '' };
                if (withoutTs) return { timestamp: null,       volunteer: withoutTs[1], details: withoutTs[2] || '' };
...
// ======================================================
// 🔹 GET ALL CENTRES (for referred centre dropdown)
// ======================================================
router.get('/centres', async (req, res) => {
    try {
        const [rows] = await pool.query(
            `SELECT Centre_id, Location, Centre_number FROM Centre ORDER BY Location ASC`
        );
        res.json(rows);
    } catch (err) {
...
// ======================================================
// 🔹 GET STRUCTURED VOLUNTEER LIST FOR AN INCIDENT
// Returns each volunteer with their Vol_assignment_id
// so the frontend can show individual Remove buttons
// ======================================================
...
// Only allowed while assignment Status = 'Pending'
// Resets volunteer Availability to Yes and clears Status
// ======================================================
router.delete('/volunteer-assignment/:id', async (req, res) => {
    ...
    try {
        ...
        // Get the row + verify dispatcher ownership through the chain
        ...
        // Ownership check
        if (row.Dispatcher_id !== req.user.id) {
            await connection.query('ROLLBACK');
            return res.status(403).json({ error: 'Access denied. This assignment does not belong to you.' });
        }

        // Block removal if assignment is Ongoing or Completed
        // Pending is the only safe window — once Ongoing the volunteer is already en route
        if (row.assignment_status === 'Ongoing' || row.assignment_status === 'Completed') {
            await connection.query('ROLLBACK');
            return res.status(400).json({
                error: `Cannot remove volunteer — assignment is already ${row.assignment_status}. Volunteers can only be removed while the assignment is Pending.`
            });
        }

        // Delete the Volunteer_assignment row
        await connection.query(
            `DELETE FROM Volunteer_assignment WHERE Vol_assignment_id = ?`,
            [volAssignmentId]
        );

        // Reset volunteer availability back to Yes and clear status
        await connection.query(
            `UPDATE Volunteer SET Availability = 'Yes', Status = NULL WHERE User_id = ?`,
            [row.Volunteer_id]
        );
        ...
        res.json({
            message:      'Volunteer removed from assignment and availability reset to Yes.',
