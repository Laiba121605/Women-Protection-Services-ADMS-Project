...
//////////////////////////////////////////////////////
// 0️⃣ PUBLIC — Forgot Password (no JWT required)
// User provides User_id and Email to submit recovery request
// Does not require login — for users who forgot their password
//////////////////////////////////////////////////////
router.post('/forgot', async (req, res) => {
    ...
    try {
        ...
        if (existing.length)
            return res.status(400).json({ message: 'You already have a pending recovery request. Please wait for an admin to process it.' });

        await conn.query('START TRANSACTION');

        // ✏️ CHANGE: only pick admins who are currently available
        const [admins] = await conn.query(
            `SELECT User_id FROM Admin WHERE Availability = 'Yes' ORDER BY User_id`
        );
        if (!admins.length) {
            await conn.query('ROLLBACK');
            return res.status(503).json({ message: 'No admins are currently available. Please try again later.' });
        }
        ...
        // ✏️ CHANGE: lock chosen admin row before inserting — prevents race condition
        // where admin logs out between being selected and the insert happening
        const [adminLock] = await conn.query(
            `SELECT User_id FROM Admin WHERE User_id = ? AND Availability = 'Yes' FOR UPDATE`,
            [assignedAdmin]
        );
        if (!adminLock.length) {
            await conn.query('ROLLBACK');
            return res.status(503).json({ message: 'Admin became unavailable, please try again.' });
        }

        const recoveryId = await generateUniqueId('Password_Recovery', 'Recovery_id');

        await conn.query(
            `INSERT INTO Password_Recovery (Recovery_id, User_email, Admin_id, Status) 
             VALUES (?, ?, ?, 'Pending')`,
        ...
        res.status(201).json({
            message: 'Password recovery request submitted successfully. An admin will reset your password shortly.',
...
//////////////////////////////////////////////////////
// 1️⃣ USER CREATES PASSWORD RECOVERY REQUEST (logged in)
// Victim, Volunteer, Dispatcher can request while logged in
//////////////////////////////////////////////////////
router.post('/', verifyJWT, allowRoles('Victim', 'Volunteer', 'Dispatcher'), async (req, res) => {
    ...
    try {
        ...
        if (!adminLock.length) {
            await conn.query("ROLLBACK");
            logger.txRollback('POST /password-recovery', `Admin became unavailable: ${assignedAdmin}`);
            return res.status(503).json({ message: 'Admin became unavailable, please try again.' });
        }
...
//////////////////////////////////////////////////////
// 3️⃣ USER SEES THEIR OWN REQUESTS
// IMPORTANT: Must be defined BEFORE /:id routes
// Otherwise Express matches 'my-requests' as an :id parameter
// Affects: Victim, Volunteer, Dispatcher dashboards
//////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////
// 4️⃣ ADMIN RESETS USER PASSWORD
// Must be before /:id to avoid conflict with PUT /:id
//////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////
// 5️⃣ ADMIN UPDATES REQUEST STATUS / NOTE
//////////////////////////////////////////////////////
